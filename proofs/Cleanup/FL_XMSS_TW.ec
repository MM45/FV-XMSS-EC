...
(* -- Adversary classes -- *)
module type Adv_EFRMA_FLXMSSTWES = {
  proc choose() : adrs
...
module EF_RMA_FLXMSSTWES(A : Adv_EFRMA_FLXMSSTWES)  = {
  proc main() : bool = {
    var ss : sseed;
    var ps : pseed;
    var ad : adrs;
...
(* 
  EF_RMA_FLXMSSTWES_NOPRF
  Essentially identical to EF_RMA_FLXMSSTWES, except that everything is inlined and all 
  WOTS_TW secret keys are sampled uniformly at random from their domain (at once) 
  instead of individually generated by a PRF.
*)
module EF_RMA_FLXMSSTWES_NOPRF(A : Adv_EFRMA_FLXMSSTWES) = {
  proc main() : bool = {
    var ps : pseed;
    var ad : adrs;    
    var root : dgstblock;
...
(* - Game sequence - *)
(* 
  EF_RMA_FLXMSSTWES -- Inlined version
  Semantically equivalent to EF_RMA_FLXMSSTWES, but all procedure calls to FL_XMSS_TW_ES
  and WOTS_TW_ES are inlined. Redundant statements arising from this inlining are removed 
*)
local module EF_RMA_FLXMSSTWES_Inline = {
  proc main() : bool = {
    var ss : sseed;
    var ps : pseed;
    var ad : adrs;    
...
(* 
  EF_RMA_FLXMSSTWES_NOPRF_LoopSnocSample
  Auxiliary game to go from sampling all WOTS-TW secret key from their domain (at once)
  to individually sampling them
*)
local module EF_RMA_FLXMSSTWES_NOPRF_LoopSnocSample = {
  proc main() : bool = {
    var ps : pseed;
    var ad : adrs;    
    var root : dgstblock;
...
(* 
  EF_RMA_FLXMSSTWES_NOPRF_Loop
  Auxiliary game to go from sampling all WOTS-TW secret key from their domain (at once)
  to individually sampling them
*)
local module EF_RMA_FLXMSSTWES_NOPRF_Loop = {
  proc main() : bool = {
    var ps : pseed;
    var ad : adrs;    
    var root : dgstblock;
...
local equiv EF_RMA_FLXMSSTWES_NOPRF_Orig_SampleSample :
  EF_RMA_FLXMSSTWES_NOPRF(A).main ~ EF_RMA_FLXMSSTWES_NOPRF_SampleSample.main 
    : ={glob A} ==> ={res}.
proof.
proc.
...
local equiv EF_RMA_FLXMSSTWES_NOPRF_SampleSample_LoopSnocSample :
  EF_RMA_FLXMSSTWES_NOPRF_SampleSample.main ~ EF_RMA_FLXMSSTWES_NOPRF_LoopSnocSample.main 
    : ={glob A} ==> ={res}.
proof.
proc.
seq 5 5 : (={glob A, ps, ad, skWOTSl, leafl}); last by sim. 
...
local equiv EF_RMA_FLXMSSTWES_NOPRF_LoopSnocSample_Loop :
  EF_RMA_FLXMSSTWES_NOPRF_LoopSnocSample.main ~ EF_RMA_FLXMSSTWES_NOPRF_Loop.main 
    : ={glob A} ==> ={res}.
proof.
proc.
seq 5 5 : (={glob A, ps, ad, skWOTSl, leafl}); last by sim. 
...
local equiv EF_RMA_FLXMSSTWES_NOPRF_Orig_Loop :
  EF_RMA_FLXMSSTWES_NOPRF(A).main ~ EF_RMA_FLXMSSTWES_NOPRF_Loop.main 
    : ={glob A} ==> ={res}.
proof.
transitivity EF_RMA_FLXMSSTWES_NOPRF_SampleSample.main 
             (={glob A} ==> ={res})
             (={glob A} ==> ={res}) => // />.
+ by move=> &2; exists (glob A){2}.
+ by apply EF_RMA_FLXMSSTWES_NOPRF_Orig_SampleSample.
transitivity EF_RMA_FLXMSSTWES_NOPRF_LoopSnocSample.main 
             (={glob A} ==> ={res})
             (={glob A} ==> ={res}) => // />.
+ by move=> &2; exists (glob A){2}.
+ by apply EF_RMA_FLXMSSTWES_NOPRF_SampleSample_LoopSnocSample.
by apply EF_RMA_FLXMSSTWES_NOPRF_LoopSnocSample_Loop.
qed.

(* 
  EF_RMA_FLXMSSTWES_NOPRF -- Variant with conditions
  Identical to EF_RMA_FLXMSSTWES_NOPRF, but keeps track of certain conditions in module variables for use in proof
*)
local module EF_RMA_FLXMSSTWES_NOPRF_Conditions = {
...
(* 
  EF_RMA_FLXMSSTWES_NOPRF -- Variant with conditions and separated sampling
  Identical to EF_RMA_FLXMSSTWES_NOPRF, but keeps track of certain conditions in module variables for use in proofs
  as well as separates sampling of messages and secret keys to facilitate the proving process in certain cases
*)
local module EF_RMA_FLXMSSTWES_NOPRF_Conditions_SSampling = {
  var valid_WOTSTWES : bool
  var coll_pkco : bool
...
(* 
  Equivalence between EF_RMA_FLXMSSTWES_NOPRF with conditions and EF_RMA_FLXMSSTWES_NOPRF with
...
*)
local equiv EF_RMA_FLXMSSTWES_NOPRF_Conditions_SSampling_Validity :
  EF_RMA_FLXMSSTWES_NOPRF_Conditions.main ~ EF_RMA_FLXMSSTWES_NOPRF_Conditions_SSampling.main : 
    ={glob A} 
    ==> 
    ={res} /\ EF_RMA_FLXMSSTWES_NOPRF_Conditions.valid_WOTSTWES{1} = EF_RMA_FLXMSSTWES_NOPRF_Conditions_SSampling.valid_WOTSTWES{2}.
proof.
proc => //=.
...
by sim : (   ={is_valid, is_fresh} 
          /\ EF_RMA_FLXMSSTWES_NOPRF_Conditions.valid_WOTSTWES{1} 
             = 
             EF_RMA_FLXMSSTWES_NOPRF_Conditions_SSampling.valid_WOTSTWES{2}).
qed.

(* 
  EF_RMA_FLXMSSTWES_NOPRF -- Variant with conditions and samplings/computations in the same while loop 
  Identical to EF_RMA_FLXMSSTWES_NOPRF, but keeps track of certain conditions in module variables for use in proof
  as well as combines sampling and computations to facilitate the proving process in certain cases
*)
local module EF_RMA_FLXMSSTWES_NOPRF_Conditions_Mono = {
  var valid_WOTSTWES : bool
  var coll_pkco : bool
...
(* 
  Equivalence between EF_RMA_FLXMSSTWES_NOPRF with conditions and separated sampling 
  and EF_RMA_FLXMSSTWES_NOPRF with conditions and combined sampling and computations; 
  includes equality on the condition concerning the validity of the WOTS-TW forgery. 
 *)
local equiv EF_RMA_FLXMSSTWES_NOPRF_Conditions_SSampling_Mono_Validity :
  EF_RMA_FLXMSSTWES_NOPRF_Conditions_SSampling.main ~ EF_RMA_FLXMSSTWES_NOPRF_Conditions_Mono.main :
    ={glob A} 
    ==> 
    ={res} /\ EF_RMA_FLXMSSTWES_NOPRF_Conditions_SSampling.valid_WOTSTWES{1} = EF_RMA_FLXMSSTWES_NOPRF_Conditions_Mono.valid_WOTSTWES{2}.
proof.
proc => //=.
...
local equiv EF_RMA_FLXMSSTWES_NOPRF_Conditions_Mono_Validity :
  EF_RMA_FLXMSSTWES_NOPRF_Conditions.main ~ EF_RMA_FLXMSSTWES_NOPRF_Conditions_Mono.main :
    ={glob A} 
    ==> 
    ={res} /\ EF_RMA_FLXMSSTWES_NOPRF_Conditions.valid_WOTSTWES{1} = EF_RMA_FLXMSSTWES_NOPRF_Conditions_Mono.valid_WOTSTWES{2}.
proof.
transitivity EF_RMA_FLXMSSTWES_NOPRF_Conditions_SSampling.main
             (={glob A} 
              ==> 
              ={res} /\ 
              EF_RMA_FLXMSSTWES_NOPRF_Conditions.valid_WOTSTWES{1}
             ...
             (={glob A} 
              ==> 
              ={res} /\ 
              EF_RMA_FLXMSSTWES_NOPRF_Conditions_SSampling.valid_WOTSTWES{1}
              =
              EF_RMA_FLXMSSTWES_NOPRF_Conditions_Mono.valid_WOTSTWES{2}) => //.
+ by move=> &1 *; exists (glob A){1} => //.
+ by apply EF_RMA_FLXMSSTWES_NOPRF_Conditions_SSampling_Validity.
...
have ->: 
  ...
  =
  Pr[EF_RMA_FLXMSSTWES_NOPRF_Loop.main() @ &m : res].
+ by byequiv EF_RMA_FLXMSSTWES_NOPRF_Orig_Loop.  
byequiv => //.
proc; inline{2} 2.
wp 20 22.
...
have ->:
  Pr[EF_RMA_FLXMSSTWES_NOPRF(A).main() @ &m : res]
  =
  Pr[EF_RMA_FLXMSSTWES_NOPRF_Conditions.main() @ &m : res].
+ byequiv => //.
  proc.
...
+ apply (ler_trans Pr[M_EF_GCMA_WOTSTWES(R_MEFGCMAWOTSTWES_EFRMAFLXMSSTWESNOPRF(A), O_MEFGCMA_WOTSTWES_NOPRF, F.O_THFC_Default).main() @ &m : res]); last by apply thm_defgcma_wotstw_noprf.
  have ->:
    ...
    =
    Pr[EF_RMA_FLXMSSTWES_NOPRF_Conditions_Mono.main() @ &m : res /\ EF_RMA_FLXMSSTWES_NOPRF_Conditions_Mono.valid_WOTSTWES].
  - by byequiv EF_RMA_FLXMSSTWES_NOPRF_Conditions_Mono_Validity.
  byequiv => //=.
  proc => //=.
  inline{2} 5.
  inline{2} 11.
  seq 12 17 : (   ={glob A, ps, root, pk, msigl, leafl}
               /\ ps{1} = O_MEFGCMA_WOTSTWES.ps{2}
               /\ ad{1} = R_MEFGCMAWOTSTWES_EFRMAFLXMSSTWESNOPRF.ad{2}
               /\ ml{1} = R_MEFGCMAWOTSTWES_EFRMAFLXMSSTWESNOPRF.ml{2}
               ...
               /\ size pkWOTSl{1} = l
               /\ size sigWOTSl{1} = l
               /\ size O_MEFGCMA_WOTSTWES.qs{2} = l
               /\ (forall (j : int), 0 <= j < l =>
                     nth witness O_MEFGCMA_WOTSTWES.qs{2} j
                     =
  ...
  - while (   ={leafl, msigl}
    ...
    * by wp; skip => />; smt(size_rcons).
    wp.
    while (   ps{1} = O_MEFGCMA_WOTSTWES.ps{2}
           /\ ad{1} = R_MEFGCMAWOTSTWES_EFRMAFLXMSSTWESNOPRF.ad{2}
           /\ ml{1} = R_MEFGCMAWOTSTWES_EFRMAFLXMSSTWESNOPRF.ml{2}
           /\ pkWOTSl{1} = R_MEFGCMAWOTSTWES_EFRMAFLXMSSTWESNOPRF.pkWOTSl{2}
           /\ sigWOTSl{1} = R_MEFGCMAWOTSTWES_EFRMAFLXMSSTWESNOPRF.sigWOTSl{2}
           ...
           /\ i0{2} = size R_MEFGCMAWOTSTWES_EFRMAFLXMSSTWESNOPRF.ml{2}
           /\ i0{2} = size R_MEFGCMAWOTSTWES_EFRMAFLXMSSTWESNOPRF.pkWOTSl{2}
           /\ i0{2} = size R_MEFGCMAWOTSTWES_EFRMAFLXMSSTWESNOPRF.sigWOTSl{2}
           /\ i0{2} = size O_MEFGCMA_WOTSTWES.qs{2}
           /\ 0 <= size ml{1} <= l
           /\ (forall (j : int), 0 <= j < size O_MEFGCMA_WOTSTWES.qs{2} =>
                 nth witness O_MEFGCMA_WOTSTWES.qs{2} j
                 =
                 (set_kpidx (set_typeidx ad{1} chtype) j,
                  nth witness ml{1} j,
    ...
    * inline{2} 2.
      ...
      move: (qs_def (index q O_MEFGCMA_WOTSTWES.qs{2}) _); first by smt(index_mem index_ge0).
...
+ byequiv => //=.
  ...
  - wp => /=.
    ...
    * wp => /=.
      ...
      move/allP: allchp => /(_ ((set_kpidx (set_typeidx R_SMDTTCRCPKCO_EFRMAFLXMSSTWESNOPRF.ad{2} pkcotype) j))) /contra /(_ _) //.
      move: (ispkcotype_setkpidx (set_typeidx R_SMDTTCRCPKCO_EFRMAFLXMSSTWESNOPRF.ad{2} pkcotype) j _ _).
    ...
    * rewrite map_rcons rcons_uniq /= uqts /=.
    ...
    move/allP: allchp => /(_ ((set_kpidx (set_typeidx R_SMDTTCRCPKCO_EFRMAFLXMSSTWESNOPRF.ad{2} pkcotype) (size R_SMDTTCRCPKCO_EFRMAFLXMSSTWESNOPRF.leafl{2})))) /contra /(_ _) //.
    move: (ispkcotype_setkpidx (set_typeidx R_SMDTTCRCPKCO_EFRMAFLXMSSTWESNOPRF.ad{2} pkcotype) (size R_SMDTTCRCPKCO_EFRMAFLXMSSTWESNOPRF.leafl{2}) _ _).
    + by apply ispkcotype_settypeidx. 
    + by smt(ge2_l).
    by smt(dist_adrstypes).  
...
seq 5 10 : (   ={m', sig', ps}
            ...
            /\ (forall (j : int), 0 <= j < l =>
                  ...
                  =
                  pkco ps{1} (set_kpidx (set_typeidx ad{1} pkcotype) j) (flatten (map DigestBlock.val (nth witness pkWOTSl{1} j))))
            /\ (forall (j : int), 0 <= j < size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.adrsll{2} =>
                  size (nth witness R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.adrsll{2} j) 
                  = 
                  size (nth witness R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2} j))
            /\ (forall (j : int), 0 <= j < size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.inpll{2} =>
                  size (nth witness R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.inpll{2} j) 
                  = 
                  size (nth witness R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2} j))
            /\ (forall (j : int), 0 <= j < size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2} =>
                  size (nth witness R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2} j) = 2 ^ (h - j - 1))
            ...
            /\ (forall (j n : int), 0 <= j < size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.adrsll{2} => 0 <= n < 2 ^ (h - j - 1) =>
                  ...
                  =
                  set_thtbidx (set_typeidx R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.ad{2} trhtype) (j + 1) n) 
            /\ (forall (j n : int), 1 <= size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.inpll{2} => 0 <= n < 2 ^ (h - 1) =>
                  nth witness (nth witness R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.inpll{2} 0) n
                  =
                  val (nth witness R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.leafl{2} (2 * n)) ++ val (nth witness R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.leafl{2} (2 * n + 1))) 
            /\ (forall (j n : int), 1 <= j < size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.inpll{2} => 0 <= n < 2 ^ (h - j - 1) =>
                  nth witness (nth witness R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.inpll{2} j) n
                  =
                  val (nth witness (nth witness R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2} (j - 1)) (2 * n)) ++ val (nth witness (nth witness R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2} (j - 1)) (2 * n + 1)))
...
+ call (: true).
  ...
  while{2} (   TRH.disj_lists (unzip1 TRH.O_SMDTTCR_Default.ts{2}) TRH.O_THFC_Default.tws{2}
            ...
            /\ 0 <= size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2} <= h
            /\ (forall (j : int), 0 <= j < size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.adrsll{2} =>
                  size (nth witness R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.adrsll{2} j) 
                  = 
                  size (nth witness R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2} j))
  ...
  - move=> _ z.
    ...
    while (   unzip1 TRH.O_SMDTTCR_Default.ts = flatten (rcons R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.adrsll adrsl)
           ...
           /\ (forall (j : int), 0 <= j < size adrsl =>
                 nth witness adrsl j
                 =
                 set_thtbidx (set_typeidx R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.ad trhtype) (size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell + 1) j)
    ...
    * rewrite size_rcons => ge1_sz1 ge0_n lt2h1_n.
      rewrite nth_rcons; case (size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2} = 0) => [eq0_sz | neq0_sz].
      + rewrite eq0_sz => /=; rewrite nlp_def 1:/# adlp_def 1:/# inplp_def 1:/#.
        by move/size_eq0: eq0_sz => ->.
      by rewrite (: 0 < size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2}) /#.
    split => [j n ge1_j + ge0_n lt2hj1_n |].
    * rewrite size_rcons=> ltsz1_j; rewrite ?nth_rcons.
      case (j < size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2}) => [ltsz_j /# | /lezNgt gesz_j].
      rewrite (: j = size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2}) 1:/# /=.
      rewrite (: size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2} - 1 < size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2}) 1:/# /=.
      rewrite nlp_def 1:/# adlp_def 1:/# inplp_def 1:/#.
      rewrite (nth_change_dfl R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.leafl{2} witness) 1:/#.
      by rewrite nth_last.
    split => [j n ge1_j + ge0_n lt2hj1_n |].
    * rewrite size_rcons=> ltsz1_j; rewrite ?nth_rcons.
      case (j < size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.adrsll{2}) => [ltsz_j /# | /lezNgt gesz_j].
      rewrite (: j = size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.adrsll{2}) 1:/# /=.
      by rewrite adlp_def /#.
    split => [n | ].
    * rewrite size_rcons => ge1_sz1 ge0_n lt2h1_n.
      rewrite nth_rcons; case (size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.inpll{2} = 0) => [eq0_sz | neq0_sz].
      + rewrite eq0_sz => /=; rewrite inplp_def 1:/#.
        by move: eq0_sz; rewrite eqszinpl_sznl size_eq0 => ->.
      by rewrite (: 0 < size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.inpll{2}) /#.
    split => [j n ge1_j + ge0_n lt2hj1_n |].
    * rewrite size_rcons=> ltsz1_j; rewrite ?nth_rcons.
      rewrite (: size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2} = size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.inpll{2}) 1:/#.
      case (j < size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.inpll{2}) => [ltsz_j /# | /lezNgt gesz_j].
      rewrite (: j = size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.inpll{2}) 1:/# /=.
      rewrite (: size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.inpll{2} - 1 < size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.inpll{2}) 1:/# /=.
      rewrite inplp_def 1:/# -nth_last.
      by rewrite (nth_change_dfl R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.leafl{2} witness) /#.
    move=> j n ge0_j + ge0_n lt2hj1_n.
    rewrite size_rcons=> ltsz1_j; rewrite nth_rcons.
    case (j < size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2}) => [ltsz_j /# | /lezNgt gesz_j].
    rewrite (: j = size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2}) 1:/# /=.
    rewrite nlp_def 1:/# adlp_def 1:/# inplp_def 1:/#.
    rewrite (: h - size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2} - 1 = h - (size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2} + 1)) 1:/#.
    rewrite (: h = height (list2tree R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.leafl{2})).
    ...
    * by rewrite (list2tree_height _ h) //; smt(size_ge0 ge1_h).
    case (size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2} = 0) => [eq0_sz | neq0_sz].
    * move/size_eq0: (eq0_sz) => -> /=.
      ...
      rewrite eql_szlfl /l (: h = h - 1 + 1) 2:exprD_nneg //; first smt(ge1_h).
      by rewrite mulzC expr1 /#.
    have eqsz_j: size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2} = j by smt().
    rewrite -nth_last -(nth_change_dfl R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.leafl{2} witness) 1:/# ?nl_valbt 1,3:/#.
    * split => [/# | _].
    ...
    * split => [/# | _].
      rewrite eqsz_j (: h - (j - 1) - 1 = h - j - 1 + 1) 1:/# exprD_nneg //; first smt(ge1_h).
      by rewrite mulzC expr1 /#.
    rewrite (: h = height (list2tree R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.leafl{2})) 2:/#.
    by rewrite (list2tree_height _ h) //; smt(ge1_h).
  ...
  by case (list2tree R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.leafl{2}).
while (   ={ps, em, idx', sigWOTS', pkWOTS'}
       /\ ad{1} = R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.ad{2}
       /\ 0 <= val idx'{1} < l
       /\ 0 <= size pkWOTS'{1} <= len
...
+ split => [| _]; first by rewrite size_ge0.
  ...
  rewrite (: size (flatten R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2}) = size (flatten (rev R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2}))). 
  - rewrite 2!size_flatten 2!StdBigop.Bigint.sumzE 2!StdBigop.Bigint.BIA.big_mapT /(\o) /=.
    by rewrite &(StdBigop.Bigint.BIA.eq_big_perm) perm_eq_rev.
  have:
    forall (j : int), 0 <= j < h => size (nth witness (rev R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2}) j) = 2 ^ j.
  ...
  rewrite (StdBigop.Bigint.BIA.big_nth witness) /(\o) /predT /= -/predT (: size (rev R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.nodell{2}) = h) 1:size_rev 1://.
...
+ rewrite (nth_map witness) => //=. 
  ...
  rewrite (: i - 1 < size R_SMDTTCRCTRH_EFRMAFLXMSSTWESNOPRF.inpll{2}) 1:/# /= => k k_rng.
...
have ->:
  Pr[EF_RMA_FLXMSSTWES(A).main() @ &m : res]
  =
  Pr[EF_RMA_FLXMSSTWES_Inline.main() @ &m : res].
+ by byequiv EFRMA_FLXMSSTWES_Inline. 
rewrite -addr0 -(subrr Pr[EF_RMA_FLXMSSTWES_NOPRF(A).main() @ &m : res]).
rewrite (addrC Pr[EF_RMA_FLXMSSTWES_NOPRF(A).main() @ &m : res]) addrA.
apply (ler_trans (`|Pr[EF_RMA_FLXMSSTWES_Inline.main() @ &m : res] -
                    Pr[EF_RMA_FLXMSSTWES_NOPRF(A).main() @ &m : res]| 
                    +
                    Pr[EF_RMA_FLXMSSTWES_NOPRF(A).main() @ &m : res])).
+ by smt(ler_trans ler_norm ler_norm_add).
rewrite -4!addrA ler_add; first by rewrite (Step_EFRMAFLXMSSTWESInlineNOPRF_PRF &m).
...
local lemma Step_EFRMA_FLXMSSTW_FLXMSSTWES &m:
  ...
  =
  Pr[EF_RMA_FLXMSSTWES(R_EFRMAFLXMSSTW_EFRMAFLXMSSTWES(A)).main() @ &m : res].
proof.
byequiv => //=.
proc.
...
end section Proof_EF_RMA_FLXMSSTW.
