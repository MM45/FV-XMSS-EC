...
require import AllCore List Distr DList DInterval IntDiv RealExp StdOrder SmtMap StdBigop BitEncoding.
require (*--*) WinternitzChecksum Word Subtype.
(*---*) import RField IntOrder RealOrder Bigreal Bigint BIA BS2Int.


(* -- Local -- *)
require (*--*) KeyedHashFunctions TweakableHashFunctions DigitalSignatures.
...
(* -- Notions -- *)
(*  Number of WOTS-TW instances to consider for D-EF-GCMA notion *)
...
type adrsindices = int * int * int * int * int * int.
...
(* 
  'Sets' chain/tree height index in given address to given i. 
  That is, if given address corresponds to indices (a, b, c, d, _, f), then
  this obtains the address corresponding to (a, b, c, d, i, f) 
*)
op set_chthidx (ad : adrs) (i : int) : adrs =
  get_adrs ((get_adidxs ad).`1, (get_adidxs ad).`2, (get_adidxs ad).`3, (get_adidxs ad).`4, i, (get_adidxs ad).`6).

(* 
  'Sets' hash/tree breadth index in given address to given i. 
  That is, if given address corresponds to indices (a, b, c, d, e, _), then
  this obtains the address corresponding to (a, b, c, d, e, i) 
*)
op set_htbidx (ad : adrs) (i : int) : adrs =
  get_adrs ((get_adidxs ad).`1, (get_adidxs ad).`2, (get_adidxs ad).`3, (get_adidxs ad).`4, (get_adidxs ad).`5, i).
...
axiom chS (g : pseed -> adrs -> dgst -> dgstblock) (ps : pseed) (ad : adrs) (s i : int) (x : dgst) :
  ...
  => 0 < i 
  => s + i <= w - 1 
  => ch g ps ad s i x = g ps (set_htbidx ad (s + i - 1)) (val (ch g ps ad s (i - 1) x)).
...
lemma ch1 (g : pseed -> adrs -> dgst -> dgstblock) (ps : pseed) (ad : adrs) (s : int) (x : dgst) : 
  ...
  => 0 <= s 
  => s + 1 <= w - 1 
  => ch g ps ad s 1 x = g ps (set_htbidx ad s) x.
...
lemma chSA (g : pseed -> adrs -> dgst -> dgstblock) (ps : pseed) (ad : adrs) (s i : int) (x : dgst) :
  ...
  => 0 < i 
  => s + i <= w - 1 
  => ch g ps ad s i x = ch g ps ad (s + 1) (i - 1) (val (g ps (set_htbidx ad s) x)).
proof.
...
have eqpfive_sethidx: 
  forall (idx : int), 0 <= idx < w - 1 => set_htbidx ad idx = set_htbidx ad' idx.
+ by move=> idx rng_idx @/set_htbidx; congr => /#.
case (i = 0) => [-> /=| neq0_i].
...
(* - Message encoding - *)
clone import WinternitzChecksum as Checksum with
      op w <- w,
  theory BaseW <- BaseW
...
(* 
  Function to turn a message into an encoded message.
  Note: encoded message is in base w and also contains checksum.
*)
op encode_msgWOTS (m : msgWOTS): emsgWOTS =
  mkemsgWOTS (encode_int len1 (bs2int (rev (val m))) len2).

lemma checksum_prop_lift (l1 l2 x x' : int) :
     0 <= l1
  => 0 <= l2
  => (w - 1) * l1 < w ^ l2
  => 0 <= x  < w ^ l1
...
move: (checksum_prop_lift len1 len2 (bs2int (rev (val m'))) (bs2int (rev (val m)))).
...
+ rewrite -negP=> /(congr1 (int2bs (8 * n))).
  rewrite -{1}(DigestBlock.valP m') -(size_rev (val m')) -{1}(DigestBlock.valP m)  -(size_rev (val m)).
...
rewrite /encode_msgWOTS !getE Hi /= !ofemsgWOTSK //.
...
op is_chwcoll (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) =
  ...
  /\ cf ps (set_chthidx ad i) (BaseW.val em'.[i]) (BaseW.val em.[i] - BaseW.val em'.[i]) (val (nth witness (val sig') i))
     <> 
...
lemma hchwcoll_hcoll (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) :
  let i = find_chwcollidx ps ad em em' sig sig' in
     cf ps (set_chthidx ad i) (BaseW.val em.[i]) (w - 1 - BaseW.val em.[i]) (val (nth witness (val sig) i)) 
     = 
     cf ps (set_chthidx ad i) (BaseW.val em'.[i]) (w - 1 - BaseW.val em'.[i]) (val (nth witness (val sig') i))
  => is_chtype ad 
  => has_chwcoll ps ad em em' sig sig' 
  => has_coll ps (set_chthidx ad i) (BaseW.val em.[i]) (BaseW.val em'.[i]) (val (nth witness (val sig) i)) (val (nth witness (val sig') i)).
proof.
...
+  move: eq_cf neq_nthsig.
   by rewrite eqw1_em /cf ch0 // 1:ischtype_setchidx 3:valP 4:valKd // => ->.
...
lemma hchwcoll_findlcollrng (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) :
  ...
  => 0 
     <= 
     find_collidx_l ps (set_chthidx ad i) (BaseW.val em.[i]) (BaseW.val em'.[i]) (val (nth witness (val sig) i)) (val (nth witness (val sig') i)) 
     < 
     w - 1 - BaseW.val em.[i].
proof.
move=> i eq_cf adch hchwcoll.
...
lemma hchwcoll_findrcollrng (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) :
  ...
  => BaseW.val em.[i] - BaseW.val em'.[i] 
     <= 
     find_collidx_r ps (set_chthidx ad i) (BaseW.val em.[i]) (BaseW.val em'.[i]) (val (nth witness (val sig) i)) (val (nth witness (val sig') i)) 
     < 
     w - 1 - BaseW.val em'.[i].
...
lemma collision_extraction (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) :
  let i = find_chwcollidx ps ad em em' sig sig' in
  let k = find_collidx_l ps (set_chthidx ad i) (BaseW.val em.[i]) (BaseW.val em'.[i]) (val (nth witness (val sig) i)) (val (nth witness (val sig') i)) in
  let l = find_collidx_r ps (set_chthidx ad i) (BaseW.val em.[i]) (BaseW.val em'.[i]) (val (nth witness (val sig) i)) (val (nth witness (val sig') i)) in
  let cl = extr_coll_l ps (set_chthidx ad i) (BaseW.val em.[i]) (BaseW.val em'.[i]) (val (nth witness (val sig) i)) (val (nth witness (val sig') i)) in
  let cr = extr_coll_r ps (set_chthidx ad i) (BaseW.val em.[i]) (BaseW.val em'.[i]) (val (nth witness (val sig) i)) (val (nth witness (val sig') i)) in
     cf ps (set_chthidx ad i) (BaseW.val em.[i]) (w - 1 - BaseW.val em.[i]) (val (nth witness (val sig) i)) 
     = 
     cf ps (set_chthidx ad i) (BaseW.val em'.[i]) (w - 1 - BaseW.val em'.[i]) (val (nth witness (val sig') i))
  => is_chtype ad
  ...
  => val cl <> val cr
     /\
     f ps (set_htbidx (set_chthidx ad i) (BaseW.val em.[i] + k)) (val cl)
     =
     f ps (set_htbidx (set_chthidx ad i) (BaseW.val em'.[i] + l)) (val cr).
proof.
move=> i k l cl cr eq_cf adch hchwcoll.
have rng_i: 0 <= i < len by apply hchwcoll_findchrng.
have hcoll: has_coll ps (set_chthidx ad i) (BaseW.val em.[i]) (BaseW.val em'.[i]) (val (nth witness (val sig) i)) (val (nth witness (val sig') i)) by apply hchwcoll_hcoll.
...
+ rewrite /cl /cr /extr_coll_l /extr_coll_r /= -/cf /find_collidx_r /find_collidx_l.
  ...
  - rewrite /cf ch0 1:ischtype_setchidx 3:valP 5:valKd // eq_sym.
    pose chn := ch _ _ _ _ _ _; move: DigestBlock.val_inj.
    by move => @/injective /(_ chn (nth witness (val sig) i)) /contra /#.
  have /(_ _) := 
    (before_find witness 
    (is_coll ps (set_chthidx ad i) (BaseW.val em.[i]) (BaseW.val em'.[i]) 
...
do 2! (rewrite /cf -chS 1:ischtype_setchidx 3:valP //; first 3 by smt(BaseW.valP)).
  ...
  flatten (map (
            fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) =>
              flatten (mkseq (fun (i : int) => 
                                ...
                                then [set_htbidx (set_chthidx admpksig.`1 i) j]
                                else []) len)) qs).
...
op relcqsad_udi_outer (ad : adrs) (m : msgWOTS) (j l : int) : adrs list =
  flatten (mkseq (fun (i : int) => 
                    ...
                    then [set_htbidx (set_chthidx ad i) j]
                    else []) l).
...
lemma relcqsad_udi_outer_cat (ad : adrs) (m : msgWOTS) (j l : int) :
  ...
  => relcqsad_udi_outer ad m j l ++ (if j < BaseW.val (encode_msgWOTS m).[l] - 1
                                     then [set_htbidx (set_chthidx ad l) j]
                                     else [])
     =
              ...
              flatten (mkseq (fun (i : int) =>
                               let em_ele = BaseW.val (encode_msgWOTS admpksig.`2).[i] in
                                ...
                                then mkseq (fun (k : int) =>
                                      set_htbidx (set_chthidx admpksig.`1 i) (j + 1 + k)) (w - 2 - j)
                                else if em_ele <> 0
                                     then mkseq (fun (k : int) =>
                                            set_htbidx (set_chthidx admpksig.`1 i) (em_ele - 1 + k)) (w - em_ele)
                                     else mkseq (fun (k : int) =>
                                            set_htbidx (set_chthidx admpksig.`1 i) (em_ele + k)) (w - 1 - em_ele)) len)) qs).
  ...
  flatten (map (
            fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) =>
              flatten (mkseq (fun (i : int) =>
                               ...
                               then mkseq 
                                    (fun (j : int) => set_htbidx (set_chthidx admpksig.`1 i) (BaseW.val (encode_msgWOTS admpksig.`2).[i] - 1 + j))
                                    (w - (BaseW.val (encode_msgWOTS admpksig.`2).[i]))
                               else mkseq 
                                    (fun (j : int) => set_htbidx (set_chthidx admpksig.`1 i) (BaseW.val (encode_msgWOTS admpksig.`2).[i] + j)) 
...
op relcqsad_tcr_outer (ad : adrs) (m : msgWOTS) (l : int) : adrs list =
  flatten (mkseq (fun (i : int) =>
                    ...
                    then mkseq 
                        (fun (j : int) => set_htbidx (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i] - 1 + j))
                        (w - (BaseW.val (encode_msgWOTS m).[i]))
                    else mkseq 
                        (fun (j : int) => set_htbidx (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i] + j)) 
...
op relcqsad_tcr_inner (ad : adrs) (em_ele : int) (k : int) : adrs list =
  if em_ele <> 0
  then mkseq 
      (fun (j : int) => set_htbidx ad (em_ele - 1 + j)) (k + 1)
  else mkseq 
      (fun (j : int) => set_htbidx ad (em_ele + j)) k.
...
lemma relcqsad_tcr_outer_inner_full (ad : adrs) (m : msgWOTS) (i : int) :
  ...
  => relcqsad_tcr_outer ad m i 
     ++ 
     relcqsad_tcr_inner (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i]) (w - 1 - (BaseW.val (encode_msgWOTS m).[i]))
     =
...
lemma relcqsad_tcr_inner_rcons (ad : adrs) (em_ele : int) (j : int) :
  ...
  => rcons (relcqsad_tcr_inner ad em_ele j) (if em_ele <> 0 
                                             then set_htbidx ad (em_ele - 1 + (j + 1))
                                             else set_htbidx ad (em_ele + j))
     =
  ...
  flatten (map (fun (admpksigxl : (adrs * msgWOTS * pkWOTS * sigWOTS) * dgstblock list) =>
                  let (admpksig, xl) = (admpksigxl.`1, admpksigxl.`2) in
                    flatten (mkseq (fun (i : int) =>
                                    if BaseW.val (encode_msgWOTS admpksig.`2).[i] <> 0
                                    then mkseq
                                         (fun (j : int) => val (cf ps (set_chthidx admpksig.`1 i) (BaseW.val (encode_msgWOTS admpksig.`2).[i] - 1) j (val (nth witness xl i))))
                                         (w - (BaseW.val (encode_msgWOTS admpksig.`2).[i]))
                                    else mkseq 
                                         (fun (j : int) => val (cf ps (set_chthidx admpksig.`1 i) (BaseW.val (encode_msgWOTS admpksig.`2).[i]) j (val (nth witness (val admpksig.`4) i))))
...
op relcqsdg_tcr_outer (ps : pseed) (ad : adrs) (m : msgWOTS) (sig : sigWOTS) (xl : dgstblock list) (l : int) : dgst list =
  flatten (mkseq (fun (i : int) =>
                  if BaseW.val (encode_msgWOTS m).[i] <> 0
                  then mkseq
                       (fun (j : int) => val (cf ps (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i] - 1) j (val (nth witness xl i))))
                       (w - (BaseW.val (encode_msgWOTS m).[i]))
                  else mkseq 
                       (fun (j : int) => val (cf ps (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i]) j (val (nth witness (val sig) i))))
                       (w - 1 - (BaseW.val (encode_msgWOTS m).[i]))) l).
...
lemma relcqsdg_tcr_outer_inner_full (ps : pseed) (ad : adrs) (m : msgWOTS) (sig : sigWOTS) (xl : dgstblock list) (i : int) :
  ...
  => relcqsdg_tcr_outer ps ad m sig xl i 
     ++ 
     relcqsdg_tcr_inner ps (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i]) (nth witness (val sig) i) (nth witness xl i) (w - 1 - (BaseW.val (encode_msgWOTS m).[i]))
     =
...
op relcqsad_pre (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) : adrs list =
  flatten (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) =>
                 flatten (mkseq (fun (i : int) => if BaseW.val (encode_msgWOTS admpksig.`2).[i] <> 0
                                                  then [set_htbidx (set_chthidx admpksig.`1 i) (BaseW.val (encode_msgWOTS admpksig.`2).[i] - 1)]
                                                  else []) len)) qs).
...
op relcqsad_pre_outer (ad : adrs) (m : msgWOTS) (l : int) : adrs list =
  flatten (mkseq (fun (i : int) => if BaseW.val (encode_msgWOTS m).[i] <> 0
                                   then [set_htbidx (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i] - 1)]
                                   else []) l).
...
op reltqsad_pre (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) : adrs list =
  flatten (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) =>
                 flatten (mkseq (fun (i : int) => 
                            mkseq (fun (j : int) =>
                              set_htbidx (set_chthidx admpksig.`1 i) (BaseW.val (encode_msgWOTS admpksig.`2).[i] + j))
                                (w - 1 - (BaseW.val (encode_msgWOTS admpksig.`2).[i]))) len)) qs).
...
op reltqsad_pre_outer (ad : adrs) (m : msgWOTS) (l : int) : adrs list =
  flatten (mkseq (fun (i : int) => 
            mkseq (fun (j : int) =>
              set_htbidx (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i] + j))
                (w - 1 - (BaseW.val (encode_msgWOTS m).[i]))) l).
...
op reltqsad_pre_inner (ad : adrs) (em_ele : int) (k : int) : adrs list =
  mkseq (fun (j : int) => set_htbidx ad (em_ele + j)) k.
...
lemma reltqsad_pre_outer_inner_full (ad : adrs) (m : msgWOTS) (i : int) :
  ...
  => reltqsad_pre_outer ad m i 
     ++ 
     reltqsad_pre_inner (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i]) (w - 1 - BaseW.val (encode_msgWOTS m).[i])  
     = 
...
lemma reltqsad_pre_inner_rcons (ad : adrs) (em_ele : int) (j : int) :
     0 <= j 
  => rcons (reltqsad_pre_inner ad em_ele j) (set_htbidx ad (em_ele + j))  
     = 
...
lemma nth_relcqsadtcr_qsadtcridx (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (i j k : int) :
  ...
  => nth witness (relcqsad_tcr qs) (qsadtcr_idx qs i j k) 
     ...
     then set_htbidx (set_chthidx q.`1 j) (BaseW.val (encode_msgWOTS q.`2).[j] - 1 + k)
     else set_htbidx (set_chthidx q.`1 j) (BaseW.val (encode_msgWOTS q.`2).[j] + k).
proof.
move=> q rng_i rng_j rng_k.
...
lemma nth_relcqsdgtcr_qsdgtcridx (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (xll : dgstblock list list) (ps : pseed) (i j k : int) :
  ...
  => nth witness (relcqsdg_tcr qs xll ps) (qsdgtcr_idx qs xll ps i j k) 
     = 
     if BaseW.val (encode_msgWOTS q.`2).[j] <> 0
     then val (cf ps (set_chthidx q.`1 j) (BaseW.val (encode_msgWOTS q.`2).[j] - 1) k (val (nth witness xl j)))
     else val (cf ps (set_chthidx q.`1 j) (BaseW.val (encode_msgWOTS q.`2).[j]) k (val (nth witness (val q.`4) j))).
proof.
...
lemma nth_relcqsadpre_qsadpreidx (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (i j : int) :
  ...
  => nth witness (relcqsad_pre qs) (qsadpre_idx qs i j)   
     = 
     set_htbidx (set_chthidx q.`1 j) (BaseW.val (encode_msgWOTS q.`2).[j] - 1).
proof.
move=> q neq0_em rng_i rng_j.
pose f := (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) =>
             (mkseq (fun (i : int) => if BaseW.val (encode_msgWOTS admpksig.`2).[i] <> 0
...
lemma uniq_relcqsadudi (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (j : int) :
     valid_hidx j
  => all is_chtype (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs)
  => uniq_preffour (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs) 
  => uniq (relcqsad_udi qs j).
proof.
move=> valj qsch uqpfqs @/relcqsad_udi.
pose f (q : adrs * msgWOTS * pkWOTS * sigWOTS) := get_preffour q.`1.
rewrite uniq_flatten_map_in => [| q q' qin qpin |] /=; last first.
+ apply (uniq_map f); move: uqpfqs => @/uniq_preffour.
...
+ rewrite allP => adl /mapP [q] [qin /= ->].
  ...
  by rewrite setchidx_sethidx_neqchidx //; smt(allP all_map).
...
rewrite setchidx_sethidx_neqpf //; first 4 by smt(allP all_map).
rewrite /eq_preffour; apply (uniq_mapP f qs) => //; first by rewrite eq_sym.
by move: uqpfqs => @/uniq_preffour; rewrite -map_comp /(\o).
qed.
...
lemma disj_relcqsadudi (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (j : int) (adl : adrs list) :
     valid_hidx j
  => all is_chtype (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs)
  => disj_preffour (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs) adl 
  => disj_lists (relcqsad_udi qs j) adl.
proof.
rewrite /disj_preffour /disj_lists /= -map_comp /(\o) => valj qsch /hasPn djpf.
rewrite hasPn => ad adin.
have neqpref_qsadl: forall q, q \in qs => ad \in adl => ! eq_preffour q.`1 ad.
+ move=> q qin adinp @/eq_preffour.
  move: (djpf (get_preffour q.`1) _).
  - by rewrite mapP; exists q.
  by apply contra => ->; rewrite mapP; exists ad.
have /#: exists q, q \in qs /\ eq_preffour q.`1 ad.
move/flatten_mapP: adin => [q] [qin /= /flatten_mapP [k [/mem_iota /= rng_k]]].
case (j < BaseW.val (encode_msgWOTS q.`2).[k] - 1) => //= ltemk_j ->.
exists q; split => [// | @/eq_preffour @/get_preffour @/set_htbidx @/set_chthidx /=].
by rewrite ?getadrsK 1,2,3:-madrs_dom //=; smt(allP all_map madidxs_valid dist_adrstypes).
qed.
...
lemma disj_relcqsadudi_reltqsadudi (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (j : int) :
     valid_hidx j
  => all is_chtype (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs) 
  => uniq_preffour (map (fun (q : adrs * msgWOTS * pkWOTS * sigWOTS) => q.`1) qs) 
...
+ move: uqpfqs; rewrite /uniq_preffour -map_comp /(\o) /= => upqfqs.
  case (j < em_ele' - 1) => [ltem1_j| /lezNgt geem1_j].
  - rewrite mapP negb_exists => m /=; rewrite negb_and -implybE => /mem_iota /= rng_m.
    by apply setchidx_sethidx_neqpf => //; smt(uniq_mapP).
  case (em_ele' <> 0) => [neq0_em | eq0_em].
  - rewrite mapP negb_exists => m /=; rewrite negb_and -implybE => /mem_iota /= rng_m.
    by apply setchidx_sethidx_neqpf => //; smt(uniq_mapP BaseW.valP).
  rewrite mapP negb_exists => m /=; rewrite negb_and -implybE => /mem_iota /= rng_m.
  by apply setchidx_sethidx_neqpf => //; smt(uniq_mapP BaseW.valP).
...
+ case (j < em_ele' - 1) => [ltem1_j| /lezNgt geem1_j].
  - rewrite mapP negb_exists => m /=; rewrite negb_and -implybE => /mem_iota /= rng_m.
    by apply setchidx_sethidx_neqchidx => // /#.
  case (em_ele' <> 0) => [neq0_em | eq0_em].
  - rewrite mapP negb_exists => m /=; rewrite negb_and -implybE => /mem_iota /= rng_m.
    by apply setchidx_sethidx_neqchidx => //; smt(BaseW.valP).
  rewrite mapP negb_exists => m /=; rewrite negb_and -implybE => /mem_iota /= rng_m.
  by apply setchidx_sethidx_neqchidx => //; smt(BaseW.valP).
move: ltemk_j; rewrite eql_k => lteml_j.
case (j < em_ele' - 1) => [ltem1_j| /lezNgt geem1_j].
+ rewrite mapP negb_exists => m /=; rewrite negb_and -implybE => /mem_iota /= rng_m.
  by apply sethidx_neq => //; smt(ischtype_setchidx dist_adrstypes).
case (em_ele' <> 0) => [neq0_em | eq0_em].
+ rewrite mapP negb_exists => m /=; rewrite negb_and -implybE => /mem_iota /= rng_m.
  by apply sethidx_neq => //; smt(ischtype_setchidx dist_adrstypes BaseW.valP).
rewrite mapP negb_exists => m /=; rewrite negb_and -implybE => /mem_iota /= rng_m.
by apply sethidx_neq => //; smt(ischtype_setchidx dist_adrstypes BaseW.valP).
qed.
...
+ rewrite allP => adl /mapP [q] [qin /= ->].
  ...
  - rewrite all_map /preim allP => i /mem_iota [ge0_i /= ltlen_i].
    ...
    * rewrite map_inj_in_uniq 2:iota_uniq => j k /mem_iota /= rng_j /mem_iota /= rng_k.
      by apply: contraLR => neqk_j; apply: sethidx_neq; smt(ischtype_setchidx allP all_map BaseW.valP).
    rewrite map_inj_in_uniq 2:iota_uniq => j k /mem_iota /= rng_j /mem_iota /= rng_k. 
    by apply: contraLR => neqk_j; apply: sethidx_neq => //=; smt(ischtype_setchidx allP all_map BaseW.valP).
  ...
  - case (BaseW.val (encode_msgWOTS q.`2).[i] <> 0) => ?; rewrite mapP negb_exists => i' /=.
    * rewrite negb_and -implybE mem_iota => ?.
      by rewrite setchidx_sethidx_neqchidx //=; smt(allP all_map BaseW.valP).
    rewrite negb_and -implybE mem_iota => ?.
    by rewrite setchidx_sethidx_neqchidx //=; smt(allP all_map BaseW.valP).
  case (BaseW.val (encode_msgWOTS q.`2).[i] <> 0) => ?; rewrite mapP negb_exists => i' /=. 
  - rewrite negb_and -implybE mem_iota => ?.
...
move: adin; have neqqpf_qppf: ! eq_preffour q.`1 q'.`1.
+ rewrite /eq_preffour; apply (uniq_mapP f qs) => //.
  by move: uqpfqs => @/uniq_preffour; rewrite -map_comp /(\o).
...
+ case (BaseW.val (encode_msgWOTS q.`2).[i] <> 0) => ?; rewrite mapP negb_exists /= => j.
  - rewrite negb_and -implybE => /mem_iota /= rng_j.
    by rewrite setchidx_sethidx_neqpf //=; smt(allP all_map BaseW.valP).
  rewrite negb_and -implybE => /mem_iota /= rng_j.
  by rewrite setchidx_sethidx_neqpf //=; smt(allP all_map BaseW.valP).
case (BaseW.val (encode_msgWOTS q.`2).[i] <> 0) => ?; rewrite mapP negb_exists /= => j.
+ rewrite negb_and -implybE => /mem_iota /= rng_j.
...
proof.
rewrite /disj_preffour /disj_lists /= -map_comp /(\o) => qsch /hasPn djpf.
rewrite hasPn => ad adin.
have neqpref_qsadl: forall q, q \in qs => ad \in adl => ! eq_preffour q.`1 ad.
+ move=> q qin adinp @/eq_preffour.
  move: (djpf (get_preffour q.`1) _).
...
proof.
move=> qsch @/uniq_preffour @/relcqsad_pre.
rewrite -map_comp /(\o) => uqpfqs.
pose f (x : adrs * msgWOTS * pkWOTS * sigWOTS) := get_preffour x.`1.
...
by apply setchidx_sethidx_neqpf; smt(allP all_map BaseW.valP uniq_mapP).
qed.
...
rewrite adval /set_htbidx /set_chthidx /=; move/allP: qsch => qsch. 
move: (qsch q.`1 _); [by rewrite mapP; exists q | move=> q1ch].
move: (qsch q'.`1 _); [by rewrite mapP; exists q' | move=> qp1ch].
rewrite ?getadrsK 3:getadrs_injA 1,2:-madrs_dom /valid_adidxs //=; first 4 by smt(allP madidxs_valid dist_adrstypes BaseW.valP).
case (q = q') => [-> /= | neqq_qp]; first by smt(BaseW.valP).
by move: uqpfqs; rewrite /uniq_preffour -map_comp /(\o) /=; smt(uniq_mapP).
qed.
...
(* Public keys *)
type pkWOTSTW = pkWOTS * pseed * chadrs.

(* Secret keys *)
type skWOTSTW = sseed * pseed * chadrs.
...
module WOTS_TW_ES = {
  ...
  proc gen_skWOTS(ss : sseed, ps : pseed, ad : adrs) : skWOTS = {
    ...
    while (size skWOTS < len) {
      skWOTS_ele <- prf_sk ss (ps, (set_htbidx (set_chthidx ad (size skWOTS))) 0);
  ...
  proc pkWOTS_from_skWOTS(skWOTS : skWOTS, ps : pseed, ad : adrs) : pkWOTS = {
    ...
    while (size pkWOTS < len) {
      ...
      *)
      pkWOTS_ele <- cf ps (set_chthidx ad (size pkWOTS)) 0 (w - 1) (val skWOTS_ele);
  ...
  (* Generate key pair *)
  proc keygen(ss : sseed, ps : pseed, chad : chadrs) : pkWOTSTW * skWOTSTW = {
    var pkWOTS : pkWOTS;
    var skWOTS : skWOTS;
    
    (* Generate secret key *)
    skWOTS <@ gen_skWOTS(ss, ps, val chad);
    
    (* Compute public key corresponding to generated secret key *) 
    pkWOTS <@ pkWOTS_from_skWOTS(skWOTS, ps, val chad);
    
    return ((pkWOTS, ps, chad), (ss, ps, chad));
  }
  ...
  proc sign(sk : skWOTSTW, m : msgWOTS) : sigWOTS = {
    ...
    while (size sig < len) {
      ...
      *)
      sig_ele <- cf ps (set_chthidx ad (size sig)) 0 em_ele (val sk_ele);
  ...
  proc pkWOTS_from_sigWOTS(m : msgWOTS, sig : sigWOTS, ps : pseed, ad : adrs) : pkWOTS = {
    ...
    while (size pkWOTS < len) {
      ...
      *)
      pkWOTS_ele <- cf ps (set_chthidx ad (size pkWOTS)) em_ele (w - 1 - em_ele) (val sig_ele);
      pkWOTS <- rcons pkWOTS pkWOTS_ele;
    }
...
(* Type of oracle given to adversaries in D-EF-GCMA game for WOTS-TW in encompassing structure *)
module type Oracle_MEFGCMA_WOTSTWES  = {
  proc init(ss_init : sseed, ps_init : pseed) : unit
  proc query(chad : chadrs, m : msgWOTS) : pkWOTS * sigWOTS
...
(* Class of adversaries against D-EF-GCMA game for WOTS-TW in encompassing structure *)
module type Adv_MEFGCMA_WOTSTWES(O : Oracle_MEFGCMA_WOTSTWES, OC : Oracle_THFC) = {
...
module M_EF_GCMA_WOTSTWES(A : Adv_MEFGCMA_WOTSTWES, O : Oracle_MEFGCMA_WOTSTWES, OC : Oracle_THFC) = {
  ...
  proc main() : bool = {
    ...
    *)
    is_valid <@ WOTS_TW_ES.verify((pkWOTS, ps, insubd ad), m', sig');
    ...
    *)
    ...
    (* 
      ...
      (6) "disj_preffour adlO adlOC": the adversary did not query the oracles O and OC with 
                                      addresses of which the corresponding indices share four-element prefixes.
    *)
    return 0 <= nrqs <= d /\ 0 <= i < nrqs /\ 
           is_valid /\ is_fresh /\ dist_preffour /\ disj_preffour adlO adlOC;     
  }
}.


(* -- Oracle implementations -- *)
(* Implementation of signing oracle given to adversary in D-EF-GCMA game for WOTS-TW in encompassing structure *)
module O_MEFGCMA_WOTSTWES : Oracle_MEFGCMA_WOTSTWES = {
  var ss : sseed
  var ps : pseed
  ...
  proc query(chad : chadrs, m : msgWOTS) : pkWOTS * sigWOTS = {
    ...
    (pk, sk) <@ WOTS_TW_ES.keygen(ss, ps, chad); 

    sig <@ WOTS_TW_ES.sign(sk, m);
        
    admpksig <- (val chad, m, pk.`1, sig);
  ...
  proc dist_addresses() : bool = {
    return uniq_preffour (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs);
  }
}.

(* Implementation of oracle given to adversary in the second game of the game sequence. *)
module O_MEFGCMA_WOTSTWES_NOPRF : Oracle_MEFGCMA_WOTSTWES = {
  include var O_MEFGCMA_WOTSTWES [-query]
  ...
  proc query(chad : chadrs, m : msgWOTS) : pkWOTS * sigWOTS = {
    ...
    while (size pk < len){
      ...
      pk_ele <- cf ps (set_chthidx ad (size pk)) 0 (w - 1) (val sk_ele);
      pk <- rcons pk pk_ele;
    }
...
(* 
  ...
  Game0_WOTSTWES and MEFGCMA_WOTSTWES without PRF
*)
module (R_PRF_Game0NOPRFWOTSTWES (A : Adv_MEFGCMA_WOTSTWES) : Adv_PRF) (O : Oracle_PRF) = {
  module O_R_PRF_Game0NOPRFWOTSTWES : Oracle_MEFGCMA_WOTSTWES = {
    include var O_MEFGCMA_WOTSTWES [-query]
    ...
    proc query(chad : chadrs, m : msgWOTS) : pkWOTS * sigWOTS = {
      ...
      while (size sk < len) {
        sk_ele <@ O.query((ps, set_htbidx (set_chthidx ad (size sk)) 0));
        sk <- rcons sk sk_ele;
      }
  ...
  proc distinguish() : bool = {
    ...
    is_valid <@ WOTS_TW_ES.verify((pk, ps, insubd ad), m', sig');
...
*)
module (R_SMDTUDC_Game23WOTSTWES (A : Adv_MEFGCMA_WOTSTWES) : Adv_SMDTUDC) (O : Oracle_SMDTUD) (OC : Oracle_THFC) = {
  module O_DistRCH = {
    var b : bool
    var ps : pseed
    var i : int
    ...
    proc query(chad : chadrs, m : msgWOTS) : dgstblock list = {
      var ad : adrs;
      var em : emsgWOTS;
      ...
      while (size chal' < len) {
        ...
        if (i < em_ele - 1) {
          chal_ele' <@ O.query(set_htbidx (set_chthidx ad (size chal')) i);
        } else {
          chal_ele' <$ ddgstblock;
        } 
      ...
      while (size chal < len) {
        ...
        if (i < em_ele - 1) {
          ...
          j <- 1;
          while (j < em_ele - 1 - i) {
            chal_ele <@ OC.query(set_htbidx (set_chthidx ad (size chal)) (i + j), val chal_ele);
            j <- j + 1;
          }
        } else {
          chal_ele <- nth witness chal' (size chal);
  ...
  module O_R_DistRCH_Game23WOTSTW : Oracle_MEFGCMA_WOTSTWES = {
    include var O_MEFGCMA_WOTSTWES [-query]
    ...
    proc query(chad : chadrs, m : msgWOTS) : pkWOTS * sigWOTS = {
      ...
      while (size pk < len) {
        ...
        if (em_ele = 0) {
          ...
          sig_ele <@ OC.query(set_htbidx (set_chthidx ad (size pk)) (em_ele - 1), val chal_ele);
        }

        (* pk_ele <- cf ps (set_chthidx ad (size pk)) em_ele (w - 1 - em_ele) sig_ele *)
        pk_ele <- sig_ele;
        j <- 0;
        while (j < w - 1 - em_ele) {
          pk_ele <@ OC.query(set_htbidx (set_chthidx ad (size pk)) (em_ele + j), val pk_ele);
          j <- j + 1;
        }
...
*)
module (R_SMDTTCRC_Game34WOTSTWES (A : Adv_MEFGCMA_WOTSTWES) : Adv_SMDTTCRC) (O : Oracle_SMDTTCR, OC : Oracle_THFC) = {
  module O_R_SMDTTCRC_Game34WOTSTWES : Oracle_MEFGCMA_WOTSTWES = {
    include var O_MEFGCMA_WOTSTWES [-init, query]
    ...
    proc query(chad : chadrs, m : msgWOTS) : pkWOTS * sigWOTS = {
      ...
      while (size pk < len) {
        ...
        if (em_ele = 0) {
          ...
          sig_ele <@ O.query(set_htbidx (set_chthidx ad (size pk)) (em_ele - 1), val x);
        }
        ...
        j <- 0;
        while (j < w - 1 - em_ele) {
          pk_ele <@ O.query(set_htbidx (set_chthidx ad (size pk)) (em_ele + j), val pk_ele);
          j <- j + 1;
        }
  ...
  proc find(ps : pseed) : int * dgst = {
    ...
    k <- find_collidx_l ps (set_chthidx ad j) (BaseW.val em.[j]) (BaseW.val em'.[j]) (val (nth witness (val sig) j)) (val (nth witness (val sig') j)) + 1;
    
    x_idx <- qsdgtcr_idx qs O_R_SMDTTCRC_Game34WOTSTWES.xll ps i j k;
    
    x' <- val (extr_coll_r ps (set_chthidx ad j) (BaseW.val em.[j]) (BaseW.val em'.[j]) (val (nth witness (val sig) j)) (val (nth witness (val sig') j)));
...
module (R_SMDTPREC_Game4WOTSTWES (A : Adv_MEFGCMA_WOTSTWES) : Adv_SMDTPREC) (O : Oracle_SMDTPRE, OC : Oracle_THFC) = {
  module O_R_SMDTPREC_Game4WOTSTWES : Oracle_MEFGCMA_WOTSTWES = {
    include var O_MEFGCMA_WOTSTWES [-query, init]
    var adl : adrs list
    ...
    proc query(chad : chadrs, m : msgWOTS) : pkWOTS * sigWOTS = {
      ...
      while (size pk < len) {
        ...
        if (em_ele = 0) {
          ...
          sig_ele <@ O.query(set_htbidx (set_chthidx ad (size pk)) (em_ele - 1));
        }
        ...
        while (j < w - 1 - em_ele) {
          pk_ele <@ OC.query(set_htbidx (set_chthidx ad (size pk)) (em_ele + j), val pk_ele);
          adl <- rcons adl (set_htbidx (set_chthidx ad (size pk)) (em_ele + j));
          j <- j + 1;
        }
  ...
  proc find(ps : pseed) : int * dgst = {
    ...
    x <- val (extr_pre ps (set_chthidx ad j) (BaseW.val em.[j]) (BaseW.val em'.[j]) (val (nth witness (val sig') j))); 
...
(* -- Proof of M-EF-GCMA for WOTS-TW (in an encompassing structure) -- *)
section Proof_M_EF_GCMA_WOTSTWES.
(* -- Auxiliary operators -- *)
(* - (Chain) Collisions - *)
(*
(* Checks whether the i-th pair of chains contain a collision *)
local op is_chwcoll (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) =
  fun (i : int) =>   
     BaseW.val em'.[i] < BaseW.val em.[i]
  /\ cf ps (set_chthidx ad i) (BaseW.val em'.[i]) (BaseW.val em.[i] - BaseW.val em'.[i]) (val (nth witness (val sig') i))
     <> 
...
(* Checks whether there is a pair of chains that contain a collision *)
local op has_chwcoll (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) : bool =
  has (is_chwcoll ps ad em em' sig sig') (range 0 len).

(* Finds index of the first chain pair in which a collision occurs *)    
local op find_chwcollidx (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) : int =
  find (is_chwcoll ps ad em em' sig sig') (range 0 len).

(* Checks whether the chain elements at index (i + em_ele) collide *)
local op is_coll (ps : pseed) (ad : adrs) (em_ele em_ele' : int) (sig_ele sig_ele' : dgst) =
  fun (i : int) =>
    cf ps ad em_ele i sig_ele = cf ps ad em_ele' (i + em_ele - em_ele') sig_ele'.

(* Checks whether there is a pair of elements in the chains that collide *)
local op has_coll (ps : pseed) (ad : adrs) (em_ele em_ele' : int) (sig_ele sig_ele' : dgst) : bool =
  has (is_coll ps ad em_ele em_ele' sig_ele sig_ele') (range 1 (w - em_ele)).

(* Finds number of iterations to apply on the original chain to obtain (one part of) the collision *)
local op find_collidx_l (ps : pseed) (ad : adrs) (em_ele em_ele' : int) (sig_ele sig_ele' : dgst) : int = 
  (find (is_coll ps ad em_ele em_ele' sig_ele sig_ele') (range 1 (w - em_ele))).

(* Finds number of iterations to apply on the forgery chain to obtain (the remaining part of) the collision *)
local op find_collidx_r (ps : pseed) (ad : adrs) (em_ele em_ele' : int) (sig_ele sig_ele' : dgst) : int = 
  find_collidx_l ps ad em_ele em_ele' sig_ele sig_ele' + em_ele - em_ele'.

(* Extracts (one part of) the collision from the original chain *)
local op extr_coll_l (ps : pseed) (ad : adrs) (em_ele em_ele' : int) (sig_ele sig_ele' : dgst) : dgstblock =
  let i = find_collidx_l ps ad em_ele em_ele' sig_ele sig_ele' in 
    cf ps ad em_ele i sig_ele.

(* Extracts (the other part of) the collision from the forgery chain *)
local op extr_coll_r (ps : pseed) (ad : adrs) (em_ele em_ele' : int) (sig_ele sig_ele' : dgst) : dgstblock =
  let i = find_collidx_r ps ad em_ele em_ele' sig_ele sig_ele' in 
    cf ps ad em_ele' i sig_ele'.
...
(* 
  ...
  find_chwcollidx is between 0 and len
*)
local lemma hchwcoll_findchrng (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) :
     has_chwcoll ps ad em em' sig sig' 
  => 0 <= find_chwcollidx ps ad em em' sig sig' < len.
proof. 
move => *; split; first by smt(find_ge0 size_range ge2_len has_find). 
...
(* 
  ...
  find_chwcollidx contain a collision
*)
local lemma hchwcoll_hcoll (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) :
  let i = find_chwcollidx ps ad em em' sig sig' in
     cf ps (set_chthidx ad i) (BaseW.val em.[i]) (w - 1 - BaseW.val em.[i]) (val (nth witness (val sig) i)) 
     = 
     cf ps (set_chthidx ad i) (BaseW.val em'.[i]) (w - 1 - BaseW.val em'.[i]) (val (nth witness (val sig') i))
...
(*
  ...
  w - 1 - BaseW.val em.[i] (excluding)
*)
local lemma hchwcoll_findlcollrng (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) :
  let i = find_chwcollidx ps ad em em' sig sig' in
     cf ps (set_chthidx ad i) (BaseW.val em.[i]) (w - 1 - BaseW.val em.[i]) (val (nth witness (val sig) i)) 
     = 
     cf ps (set_chthidx ad i) (BaseW.val em'.[i]) (w - 1 - BaseW.val em'.[i]) (val (nth witness (val sig') i))
...
(*
  ...
  BaseW.val em.[i] - BaseW.val em'.[i] (including) and w - 1 - BaseW.val em'.[i] (excluding)
*)
local lemma hchwcoll_findrcollrng (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) :
  let i = find_chwcollidx ps ad em em' sig sig' in
     cf ps (set_chthidx ad i) (BaseW.val em.[i]) (w - 1 - BaseW.val em.[i]) (val (nth witness (val sig) i)) 
     = 
     cf ps (set_chthidx ad i) (BaseW.val em'.[i]) (w - 1 - BaseW.val em'.[i]) (val (nth witness (val sig') i)) 
...
(*
  ...
  via extr_coll_l and extr_coll_r
*)
local lemma collision_extraction (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) :
  let i = find_chwcollidx ps ad em em' sig sig' in
  let k = find_collidx_l ps (set_chthidx ad i) (BaseW.val em.[i]) (BaseW.val em'.[i]) (val (nth witness (val sig) i)) (val (nth witness (val sig') i)) in
  let l = find_collidx_r ps (set_chthidx ad i) (BaseW.val em.[i]) (BaseW.val em'.[i]) (val (nth witness (val sig) i)) (val (nth witness (val sig') i)) in
  let cl = extr_coll_l ps (set_chthidx ad i) (BaseW.val em.[i]) (BaseW.val em'.[i]) (val (nth witness (val sig) i)) (val (nth witness (val sig') i)) in
...
(* - (Chain) Preimages - *)
(* Checks whether there is a preimage in the i-th chain *)
local op is_chwpre (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) =
  fun (i : int),   
     BaseW.val em'.[i] < BaseW.val em.[i]
  /\ cf ps (set_chthidx ad i) (BaseW.val em'.[i]) (BaseW.val em.[i] - BaseW.val em'.[i]) (val (nth witness (val sig') i))
     = 
...
(* Checks whether there is a chain that contains a preimage *)   
local op has_chwpre (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) =
  has (is_chwpre ps ad em em' sig sig') (range 0 len).

(* Finds the index of first chain that contains a preimage *)
local op find_chwpreidx (ps : pseed) (ad : adrs) (em em' : emsgWOTS) (sig sig' : sigWOTS) : int = 
  find (is_chwpre ps ad em em' sig sig') (range 0 len).

(* Extracts a preimage from the i-th chain *)
local op extr_pre (ps : pseed) (ad : adrs) (em_ele em_ele' : int) (sig_ele' : dgst) : dgstblock =
  cf ps ad em_ele' (em_ele - em_ele' - 1) sig_ele'. 
...
(* 
  ...
  then there exists a chain preimage
*)
local lemma nhchwcoll_hchwpre (ps : pseed) (ad : adrs) (m m' : msgWOTS) (sig sig' : sigWOTS) :
     m <> m'
  => !has_chwcoll ps ad (encode_msgWOTS m) (encode_msgWOTS m') sig sig'
  => has_chwpre ps ad (encode_msgWOTS m) (encode_msgWOTS m') sig sig'.
proof.
...
(* If there is chain that contains a preimage, then find_chwpreidx is between 0 and len *)
local lemma hchwpre_findprerng (ps : pseed) (ad : adrs) (m m' : msgWOTS) (sig sig' : sigWOTS) :
     has_chwpre ps ad (encode_msgWOTS m) (encode_msgWOTS m') sig sig'
  => 0 <= find_chwpreidx ps ad (encode_msgWOTS m) (encode_msgWOTS m') sig sig' < len.
proof.
move=> hchwpre; rewrite find_ge0 /= /find_chwpreidx {2}(: len = size (range 0 len)).
...
(* 
  ...
  find_chwpreidx is not 0 
*)
local lemma hchwpre_neq0_findchwpre (ps : pseed) (ad : adrs) (m m' : msgWOTS) (sig sig' : sigWOTS) :
     has_chwpre ps ad (encode_msgWOTS m) (encode_msgWOTS m') sig sig'
  => BaseW.val (encode_msgWOTS m).[find_chwpreidx ps ad (encode_msgWOTS m) (encode_msgWOTS m') sig sig'] <> 0.
proof.
move=> hchwpre; move/(nth_find witness): (hchwpre) => @/find_chwpreidx @/is_chwpre /=.
...
(* - Relations between oracle queries in reductions - *)
(* Overall relation between addresses in signing oracle and challenge oracle queries in SM-DT-UD hybrid reduction *)
local op relcqsad_udi (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (j : int) : adrs list =
  flatten (map (
            fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) =>
              flatten (mkseq (fun (i : int) => 
                                if j < BaseW.val (encode_msgWOTS admpksig.`2).[i] - 1
...
(* Intermediate relation (while-loop) between addresses in signing oracle and challenge oracle queries in SM-DT-UD hybrid reduction *)
local op relcqsad_udi_outer (ad : adrs) (m : msgWOTS) (j l : int) : adrs list =
  flatten (mkseq (fun (i : int) => 
                    if j < BaseW.val (encode_msgWOTS m).[i] - 1
                    then [set_htbidx (set_chthidx ad i) j]
                    else []) l).
...
(* 
  ...
  public keys and signatures of the queries in the query list 
*)
local lemma relcqsadudi_rcons_qs (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (j : int) (ad : adrs) (m : msgWOTS) (pk pk' : pkWOTS) (sig sig' : sigWOTS) :
  relcqsad_udi (rcons qs (ad, m, pk, sig)) j = relcqsad_udi (rcons qs (ad, m, pk', sig')) j.
proof. by rewrite /relcqsad_udi; congr; rewrite 2!map_rcons. qed.

(* Interaction between relcqsad_udi and relcqsad_udi_outer *)                   
local lemma relcqsad_udi_outer_full (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (j : int) (ad : adrs) (m : msgWOTS) (pk : pkWOTS) (sig : sigWOTS) :
  relcqsad_udi qs j ++ relcqsad_udi_outer ad m j len
  =
  relcqsad_udi (rcons qs (ad, m, pk, sig)) j.
proof. by rewrite /relcqsad_udi -flatten_rcons; congr; rewrite map_rcons. qed.
...
(* Extension of relcqsad_udi_outer ad m j l to relcqsad_udi_outer ad m j (l + 1) *)
local lemma relcqsad_udi_outer_cat (ad : adrs) (m : msgWOTS) (j l : int) :
     0 <= l
  => relcqsad_udi_outer ad m j l ++ (if j < BaseW.val (encode_msgWOTS m).[l] - 1
                                     then [set_htbidx (set_chthidx ad l) j]
                                     else [])
...
(* Range in which relcqsad_udi qs j lies *)
local lemma relcqsadudi_rng (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (j : int) :
  0 <= size (relcqsad_udi qs j) <= size qs * len.
proof.
rewrite /relcqsad_udi size_flatten sumzE 2!big_map /(\o) /= /predT -/predT.
split => [| _].
...
(* Overall (partial) relation between addresses in signing oracle and family oracle queries in SM-DT-UD hybrid reduction *)
local op reltqsad_udi (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (j : int) : adrs list =
  flatten (map (
            fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) =>
              flatten (mkseq (fun (i : int) =>
                               let em_ele = BaseW.val (encode_msgWOTS admpksig.`2).[i] in
...
(* 
  ...
  public keys and signatures of the queries in the query list 
*)                                          
local lemma reltqsadudi_rcons_qs (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (j : int) (ad : adrs) (m : msgWOTS) (pk pk' : pkWOTS) (sig sig' : sigWOTS) :
  reltqsad_udi (rcons qs (ad, m, pk, sig)) j = reltqsad_udi (rcons qs (ad, m, pk', sig')) j.
proof. by rewrite /reltqsad_udi; congr; rewrite 2!map_rcons. qed.
                                                   
(* Overall relation between addresses in signing oracle and challenge oracle queries in SM-DT-TCR reduction *)
local op relcqsad_tcr (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) : adrs list =
  flatten (map (
            fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) =>
              flatten (mkseq (fun (i : int) =>
                               if BaseW.val (encode_msgWOTS admpksig.`2).[i] <> 0
...
(* Intermediate relation (outer while-loop) between addresses in signing oracle and challenge oracle queries in SM-DT-TCR reduction *)
local op relcqsad_tcr_outer (ad : adrs) (m : msgWOTS) (l : int) : adrs list =
  flatten (mkseq (fun (i : int) =>
                    if BaseW.val (encode_msgWOTS m).[i] <> 0
                    then mkseq 
                        (fun (j : int) => set_htbidx (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i] - 1 + j))
...
(* Intermediate relation (inner while-loop) between addresses in signing oracle and challenge oracle queries in SM-DT-TCR reduction *)
local op relcqsad_tcr_inner (ad : adrs) (em_ele : int) (k : int) : adrs list =
  if em_ele <> 0
  then mkseq 
      (fun (j : int) => set_htbidx ad (em_ele - 1 + j)) (k + 1)
  else mkseq 
...
(* Interaction between relcqsad_tcr and relcqsad_tcr_outer *)
local lemma relcqsad_tcr_outer_full (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (ad : adrs) (m : msgWOTS) (pk : pkWOTS) (sig : sigWOTS) :
  relcqsad_tcr qs ++ relcqsad_tcr_outer ad m len
  =
  relcqsad_tcr (rcons qs (ad, m, pk, sig)).
proof. by rewrite /relcqsad_tcr -flatten_rcons; congr; rewrite map_rcons. qed.
...
(* Interaction between relcqsad_tcr_outer and relcqsad_tcr_inner *)
local lemma relcqsad_tcr_outer_inner_full (ad : adrs) (m : msgWOTS) (i : int) :
     0 <= i 
  => relcqsad_tcr_outer ad m i 
     ++ 
     relcqsad_tcr_inner (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i]) (w - 1 - (BaseW.val (encode_msgWOTS m).[i]))
...
(* Extension of relcqsad_tcr_inner ad em_ele j to relcqsad_tcr_inner ad em_ele (j + 1) *)
local lemma relcqsad_tcr_inner_rcons (ad : adrs) (em_ele : int) (j : int) :
     0 <= j 
  => rcons (relcqsad_tcr_inner ad em_ele j) (if em_ele <> 0 
                                             then set_htbidx ad (em_ele - 1 + (j + 1))
                                             else set_htbidx ad (em_ele + j))
...
(* Range in which relcqsad_tcr qs lies *)
local lemma relcqsadtcr_rng (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) :
  size qs <= size (relcqsad_tcr qs) <= size qs * len * w.
proof.
rewrite -count_predT -(Ring.IntID.mul1r (count _ _)) -big_constz.
rewrite /relcqsad_tcr size_flatten sumzE 2!big_map /(\o) /= /predT -/predT. 
...
(* Overall relation between digests in signing oracle and challenge oracle queries in SM-DT-TCR-C reduction *)
local op relcqsdg_tcr (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (xll : dgstblock list list) (ps : pseed)  : dgst list =
  flatten (map (fun (admpksigxl : (adrs * msgWOTS * pkWOTS * sigWOTS) * dgstblock list) =>
                  let (admpksig, xl) = (admpksigxl.`1, admpksigxl.`2) in
                    flatten (mkseq (fun (i : int) =>
                                    if BaseW.val (encode_msgWOTS admpksig.`2).[i] <> 0
...
(* Intermediate relation (outer while-loop) between digests in signing oracle and challenge oracle queries in SM-DT-TCR-C reduction *)
local op relcqsdg_tcr_outer (ps : pseed) (ad : adrs) (m : msgWOTS) (sig : sigWOTS) (xl : dgstblock list) (l : int) : dgst list =
  flatten (mkseq (fun (i : int) =>
                  if BaseW.val (encode_msgWOTS m).[i] <> 0
                  then mkseq
                       (fun (j : int) => val (cf ps (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i] - 1) j (val (nth witness xl i))))
...
(* Intermediate relation (inner while-loop) between digests in signing oracle and challenge oracle queries in SM-DT-TCR-C reduction *)
local op relcqsdg_tcr_inner (ps : pseed) (ad : adrs) (em_ele : int) (sig_ele : dgstblock) (x : dgstblock) (k : int) : dgst list =
  if em_ele <> 0
  then mkseq
       (fun (j : int) => val (cf ps ad (em_ele - 1) j (val x))) (k + 1)
  else mkseq 
...
(* Interaction between relcqsdg_tcr and relcqsdg_tcr_outer *)
local lemma relcqsdg_tcr_outer_full (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (xll : dgstblock list list) 
                                    (ps : pseed) (ad : adrs) (m : msgWOTS) (pk : pkWOTS) (sig : sigWOTS) (xl : dgstblock list) :
     size qs = size xll 
  => relcqsdg_tcr qs xll ps ++ relcqsdg_tcr_outer ps ad m sig xl len
     =
...
(* Interaction between relcqsdg_tcr_outer and relcqsdg_tcr_inner *)
local lemma relcqsdg_tcr_outer_inner_full (ps : pseed) (ad : adrs) (m : msgWOTS) (sig : sigWOTS) (xl : dgstblock list) (i : int) :
     0 <= i 
  => relcqsdg_tcr_outer ps ad m sig xl i 
     ++ 
     relcqsdg_tcr_inner ps (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i]) (nth witness (val sig) i) (nth witness xl i) (w - 1 - (BaseW.val (encode_msgWOTS m).[i]))
...
(* Extension of relcqsdg_tcr_inner ps ad em_ele sig_ele x j to relcqsdg_tcr_inner ps ad em_ele sig_ele x (j + 1) *)
local lemma relcqsdg_tcr_inner_rcons (ps : pseed) (ad : adrs) (em_ele : int) (sig_ele : dgstblock) (x : dgstblock) (j : int) :
  0 <= j 
  => rcons (relcqsdg_tcr_inner ps ad em_ele sig_ele x j) 
           (if em_ele <> 0
            then val (cf ps ad (em_ele - 1) (j + 1) (val x))
...
(* Overall relation between addresses in signing oracle and challenge oracle queries in SM-DT-PRE-C reduction *)
local op relcqsad_pre (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) : adrs list =
  flatten (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) =>
                 flatten (mkseq (fun (i : int) => if BaseW.val (encode_msgWOTS admpksig.`2).[i] <> 0
                                                  then [set_htbidx (set_chthidx admpksig.`1 i) (BaseW.val (encode_msgWOTS admpksig.`2).[i] - 1)]
                                                  else []) len)) qs).
...
(* Intermediate relation (outer while-loop) between addresses in signing oracle and challenge oracle queries in SM-DT-PRE-C reduction *)
local op relcqsad_pre_outer (ad : adrs) (m : msgWOTS) (l : int) : adrs list =
  flatten (mkseq (fun (i : int) => if BaseW.val (encode_msgWOTS m).[i] <> 0
                                   then [set_htbidx (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i] - 1)]
                                   else []) l).
...
(* Interaction between relcqsad_pre and relcqsad_pre_outer *)                                   
local lemma relcqsad_pre_outer_full (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (ad : adrs) (m : msgWOTS) (pk : pkWOTS) (sig : sigWOTS) :
  relcqsad_pre qs ++ relcqsad_pre_outer ad m len = relcqsad_pre (rcons qs (ad, m, pk, sig)).
proof. by rewrite /relcqsad_pre /relcqsad_pre_outer map_rcons /= flatten_rcons. qed.
                                 
(* Overall relation between digests in signing oracle and challenge oracle queries in SM-DT-PRE reduction *)
local op relcqsdg_pre (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) : dgstblock list =
  flatten (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) =>
                 flatten (mkseq (fun (i : int) => if BaseW.val (encode_msgWOTS admpksig.`2).[i] <> 0
                                                  then [nth witness (val admpksig.`4) i]
                                                  else []) len)) qs).
...
(* Intermediate relation (outer while-loop) between addresses in signing oracle and challenge oracle queries in SM-DT-PRE-C reduction *)
local op relcqsdg_pre_outer (m : msgWOTS) (sig : dgstblock list) (l : int) : dgstblock list =
  flatten (mkseq (fun (i : int) => if BaseW.val (encode_msgWOTS m).[i] <> 0
                                   then [nth witness sig i]
                                   else []) l).

(* Interaction between relcqsdg_pre and relcqsdg_pre_outer *) 
local lemma relcqsdg_pre_outer_full (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (ad : adrs) (m : msgWOTS) (pk : pkWOTS) (sig : sigWOTS) :
  relcqsdg_pre qs ++ relcqsdg_pre_outer m (val sig) len = relcqsdg_pre (rcons qs (ad, m, pk, sig)).
proof. by rewrite /relcqsdg_pre /relcqsdg_pre_outer map_rcons /= flatten_rcons. qed.

(* Range in which relcqsad_pre lies *)
local lemma relcqsadpre_rng (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) :
  size qs <= size (relcqsad_pre qs) <= size qs * len.
proof.
rewrite -count_predT -(Ring.IntID.mul1r (count _ _)) -big_constz.
rewrite /relcqsad_pre size_flatten sumzE 2!big_map /(\o) /= /predT -/predT.
...
(* Overall relation between addresses in signing oracle and family oracle queries in SM-DT-PRE reduction *)
local op reltqsad_pre (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) : adrs list =
  flatten (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) =>
                 flatten (mkseq (fun (i : int) => 
                            mkseq (fun (j : int) =>
                              set_htbidx (set_chthidx admpksig.`1 i) (BaseW.val (encode_msgWOTS admpksig.`2).[i] + j))
...
(* Intermediate relation (outer while-loop) between addresses in signing oracle and family oracle queries in SM-DT-PRE reduction *)
local op reltqsad_pre_outer (ad : adrs) (m : msgWOTS) (l : int) : adrs list =
  flatten (mkseq (fun (i : int) => 
            mkseq (fun (j : int) =>
              set_htbidx (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i] + j))
                (w - 1 - (BaseW.val (encode_msgWOTS m).[i]))) l).
...
(* Intermediate relation (inner while-loop) between addresses in signing oracle and family oracle queries in SM-DT-PRE reduction *)
local op reltqsad_pre_inner (ad : adrs) (em_ele : int) (k : int) : adrs list =
  mkseq (fun (j : int) => set_htbidx ad (em_ele + j)) k.

(* Interaction between reltqsad_pre and reltqsad_pre_outer *)
local lemma reltqsad_pre_outer_full (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (ad : adrs) (m : msgWOTS) (pk : pkWOTS) (sig : sigWOTS) :
  reltqsad_pre qs ++ reltqsad_pre_outer ad m len = reltqsad_pre (rcons qs (ad, m, pk, sig)).
proof. by rewrite /reltqsad_pre /reltqsad_pre_outer map_rcons /= flatten_rcons. qed.

(* Interaction between reltqsad_pre_outer and reltqsad_pre_inner *)
local lemma reltqsad_pre_outer_inner_full (ad : adrs) (m : msgWOTS) (i : int) :
     0 <= i 
  => reltqsad_pre_outer ad m i 
     ++ 
     reltqsad_pre_inner (set_chthidx ad i) (BaseW.val (encode_msgWOTS m).[i]) (w - 1 - BaseW.val (encode_msgWOTS m).[i])  
...
(* Extension of reltqsad_pre_inner ad em_ele j to reltqsad_pre_inner ad em_ele (j + 1)  *)
local lemma reltqsad_pre_inner_rcons (ad : adrs) (em_ele : int) (j : int) :
     0 <= j 
  => rcons (reltqsad_pre_inner ad em_ele j) (set_htbidx ad (em_ele + j))  
     = 
     reltqsad_pre_inner ad em_ele (j + 1).
...
(* Computes index in query list of tweakable hash function oracles corresponding to extracted collision/preimage *)
local op qs_idx (f : 'a -> 'b list list) (s : 'a list) (i j k : int) : int =
  sumz (map (fun x => sumz (map size x)) (take i (map f s))) + sumz (map size (take j (nth witness (map f s) i))) + k.

(* Computes index of address in query list of challenge oracle in SM-DT-TCR-C reduction corresponding to extracted collision *)
local op qsadtcr_idx (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (i j k : int) : int =
  let f = (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) =>
            (mkseq (fun (i : int) =>
                    if BaseW.val (encode_msgWOTS admpksig.`2).[i] <> 0
                    then mkseq 
...
(* Computes index of digest in query list of challenge oracle in SM-DT-TCR-C reduction corresponding to extracted collision *)
local op qsdgtcr_idx (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (xll : dgstblock list list) (ps : pseed) (i j k : int) : int =
  let f = (fun (admpksigxl : (adrs * msgWOTS * pkWOTS * sigWOTS) * dgstblock list) =>
                  let (admpksig, xl) = (admpksigxl.`1, admpksigxl.`2) in
                    (mkseq (fun (i : int) =>
                            if BaseW.val (encode_msgWOTS admpksig.`2).[i] <> 0
...
(* Computes index of address in query list of challenge oracle in SM-DT-TCR-C reduction corresponding to extracted preimage *)
local op qsadpre_idx (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (i j : int) : int =
  let f = (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) =>
             (mkseq (fun (i : int) => if BaseW.val (encode_msgWOTS admpksig.`2).[i] <> 0
                                      then [set_htbidx (set_chthidx admpksig.`1 i) (BaseW.val (encode_msgWOTS admpksig.`2).[i] - 1)]
                                      else []) len)) in
...
(* Computes index of digest in query list of challenge oracle in SM-DT-TCR-C reduction corresponding to extracted preimage *)
local op qsdgpre_idx (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (i j : int) : int =
  let f = (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) =>
             (mkseq (fun (i : int) => if BaseW.val (encode_msgWOTS admpksig.`2).[i] <> 0
                                      then [nth witness (val admpksig.`4) i]
                                      else []) len)) in
...
(* Equality between qsadtcr_idx qs i j k and qsdgtcr_idx qs xll ps i j k *)
local lemma eq_qsadtcridx_qsdgtcridx (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (xll : dgstblock list list) (ps : pseed) (i j k : int) :
     size qs = size xll
  => 0 <= i < size qs
  => qsadtcr_idx qs i j k = qsdgtcr_idx qs xll ps i j k.
proof.
...
(* Equality between qsadpre_idx qs i j and qsdgpre_idx qs i j *)
local lemma eq_qsadpreidx_qsdgpreidx (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (i j : int) :
     0 <= i < size qs 
  => qsadpre_idx qs i j = qsdgpre_idx qs i j.
proof.
move=> rng_i.
...
(* Range in which qsadtcr_idx lies *)
local lemma qsadtcridx_rng (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (i j k : int) :
  let q = nth witness qs i in
     0 <= i < size qs
  => 0 <= j < len
  => 0 <= k < (if BaseW.val (encode_msgWOTS q.`2).[j] <> 0 
...
(* Value of the element in relcqsad_tcr qs at index qsadtcr_idx qs i j k  *)
local lemma nth_relcqsadtcr_qsadtcridx (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (i j k : int) :
  let q = nth witness qs i in
     0 <= i < size qs
  => 0 <= j < len
  => 0 <= k < (if BaseW.val (encode_msgWOTS q.`2).[j] <> 0 
...
(* Range in which qsdgtcr_idx qs xll ps i j k lies *)
local lemma qsdgtcridx_rng (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (xll : dgstblock list list) (ps : pseed) (i j k : int) :
  let q = nth witness qs i in
     size qs = size xll
  => 0 <= i < size qs
  => 0 <= j < len
...
(* Value of the element in relcqsdg_tcr qs xll ps at index qsdgtcr_idx qs xll ps i j k  *)
local lemma nth_relcqsdgtcr_qsdgtcridx (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (xll : dgstblock list list) (ps : pseed) (i j k : int) :
  let q = nth witness qs i in
  let xl = nth witness xll i in
     size qs = size xll
  => 0 <= i < size qs
...
(* Range in which qsadpre_idx qs xll ps i j k lies *)
local lemma qsadpreidx_rng (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (i j : int) :
  let q = nth witness qs i in
     BaseW.val (encode_msgWOTS q.`2).[j] <> 0
  => 0 <= i < size qs 
  => 0 <= j < len
...
(* Value of the element in relcqsad_pre qs at index qsadpre_idx qs i j *)
local lemma nth_relcqsadpre_qsadpreidx (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (i j : int) :
  let q = nth witness qs i in
     BaseW.val (encode_msgWOTS q.`2).[j] <> 0
  => 0 <= i < size qs 
  => 0 <= j < len 
...
(* Range in which qsdgpre_idx qs i j lies *)
local lemma qsdgpreidx_rng (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (i j : int) :
  let q = nth witness qs i in
     BaseW.val (encode_msgWOTS q.`2).[j] <> 0
  => 0 <= i < size qs 
  => 0 <= j < len
...
(* Value of the element in relcqsdg_pre qs at index qsdgpre_idx qs i j *)
local lemma nth_relcqsdgpre_qsdgpreidx (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (i j : int) :
  let q = nth witness qs i in
     BaseW.val (encode_msgWOTS q.`2).[j] <> 0
  => 0 <= i < size qs 
  => 0 <= j < len 
...
(* - Uniqueness and disjointness for query lists of tweakable hash function oracles  - *)
(* Uniqueness of relcqsad_udi qs j *)
local lemma uniq_relcqsadudi (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (j : int) :
     valid_hidx j
  => all is_chtype (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs)
  => uniq_preffour (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs) 
  => uniq (relcqsad_udi qs j).
...
(* Disjointness of relcqsad_udi qs j and arbitrary list *)
local lemma disj_relcqsadudi (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (j : int) (adl : adrs list) :
     valid_hidx j
  => all is_chtype (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs)
  => disj_preffour (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs) adl 
  => disj_lists (relcqsad_udi qs j) adl.
...
(* Disjointness of relcqsad_udi qs j and reltqsad_udi qs j *)
local lemma disj_relcqsadudi_reltqsadudi (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (j : int) :
     valid_hidx j
  => all is_chtype (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs) 
  => uniq_preffour (map (fun (q : adrs * msgWOTS * pkWOTS * sigWOTS) => q.`1) qs) 
  => disj_lists (relcqsad_udi qs j) (reltqsad_udi qs j).
...
(* Uniqueness of relcqsad_tcr qs *)
local lemma uniq_relcqsadtcr (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) :
     all is_chtype (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs)
  => uniq_preffour (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs) 
  => uniq (relcqsad_tcr qs).
proof.
...
(* Disjointness of relcqsad_tcr qs and arbitrary list *)  
local lemma disj_relcqsadtcr (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) (adl : adrs list) :
     all is_chtype (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs)
  => disj_preffour (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs) adl 
  => disj_lists (relcqsad_tcr qs) adl.
proof.
...
(* Uniqueness of relcqsad_pre qs *)
local lemma uniq_relcqsadpre (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) :
  all is_chtype (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs) => 
  uniq_preffour (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs) => 
  uniq (relcqsad_pre qs).
proof.
...
(* Disjointness of relcqsad_pre qs and reltqsad_pre qs *)
local lemma disj_relcqsadpre_reltqsadpre (qs : (adrs * msgWOTS * pkWOTS * sigWOTS) list) :
     all is_chtype (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) qs) 
  => uniq_preffour (map (fun (q : adrs * msgWOTS * pkWOTS * sigWOTS) => q.`1) qs) 
  => disj_lists (relcqsad_pre qs) (reltqsad_pre qs).
proof.
...
(* -- Module declarations -- *)
declare module A <: Adv_MEFGCMA_WOTSTWES { (* Assumptions *)
  -O_MEFGCMA_WOTSTWES, -O_PRF_Default, -O_SMDTUD_Default, -O_SMDTTCR_Default, -O_SMDTPRE_Default, -O_THFC_Default,
  -R_SMDTUDC_Game23WOTSTWES, -R_SMDTTCRC_Game34WOTSTWES, -R_SMDTPREC_Game4WOTSTWES, -R_PRF_Game0NOPRFWOTSTWES (* Reductions *)
}.

declare axiom A_choose_ll (O <: Oracle_MEFGCMA_WOTSTWES {-A} ) (OC <: Oracle_THFC {-A} ) :
  islossless O.query => islossless OC.query => islossless A(O, OC).choose.

declare axiom A_forge_ll (O <: Oracle_MEFGCMA_WOTSTWES {-A} ) (OC <: Oracle_THFC {-A} ) :
  islossless A(O, OC).forge.


(* -- Oracle implementations -- *)
(* Implementation of oracle given to adversary in Game0_WOTSTWES. *)
local module O_Game0_WOTSTWES : Oracle_MEFGCMA_WOTSTWES = {
  include var O_MEFGCMA_WOTSTWES [-query]
  ...
  proc query(chad : chadrs, m : msgWOTS) : pkWOTS * sigWOTS = {
    ...
    sk <- [];
    while (size sk < len) {
      sk_ele <- prf_sk ss (ps, (set_htbidx (set_chthidx ad (size sk)) 0));
      sk <- rcons sk sk_ele;
    }
...
*)
local module O_Game2_WOTSTWES : Oracle_MEFGCMA_WOTSTWES = {
  include var O_MEFGCMA_WOTSTWES [-query]
...
*)
local module O_Game34_WOTSTWES : Oracle_MEFGCMA_WOTSTWES = {
  include var O_MEFGCMA_WOTSTWES [-query]
  ...
  proc query(chad : chadrs, m : msgWOTS) : pkWOTS * sigWOTS = {
    ...
    while (size sig < len) {
      ...
      if (em_ele = 0) { 
        ...
        sig_ele <- cf ps (set_chthidx ad (size sig)) (em_ele - 1) 1 (val sk_ele);
      }
    ...
    while (size pk < len){
      ...
      em_ele <- BaseW.val em.[size pk];
      pk_ele <- cf ps (set_chthidx ad (size pk)) em_ele (w - 1 - em_ele) (val sig_ele);
      pk <- rcons pk pk_ele;
    }
  ...
  proc query(chad : chadrs, m : msgWOTS) : dgstblock list = {
    ...
    if (b) {
      ...
      while (size chal < len) {
        chal_ele' <- nth witness chal' (size chal);
        em_ele <- BaseW.val em.[size chal];
        chal_ele <- cf ps (set_chthidx ad (size chal)) 0 (em_ele - 1) (val chal_ele');
        chal <- rcons chal chal_ele;
      }
    }
...
(* 
  Auxiliary implementations for O_MEFGCMA_WOTSTWES_NOPRF, used to obtain the desired form
  for the proof of the PRF reduction 
*)  
local module O_MEFGCMA_WOTSTWES_NOPRF_SampleSample : Oracle_MEFGCMA_WOTSTWES = {
  include var O_MEFGCMA_WOTSTWES [-query]
...
local module O_MEFGCMA_WOTSTWES_NOPRF_LoopSnocSample : Oracle_MEFGCMA_WOTSTWES = {
  include var O_MEFGCMA_WOTSTWES [-query]
...
local module O_MEFGCMA_WOTSTWES_NOPRF_Alt : Oracle_MEFGCMA_WOTSTWES = {
  include var O_MEFGCMA_WOTSTWES [-query]
...
(* Equivalences between original and auxiliary implementations of O_MEFGCMA_WOTSTWES_NOPRF *)
local equiv O_MEFGCMA_WOTSTWES_NOPRF_Orig_SampleSample : 
  O_MEFGCMA_WOTSTWES_NOPRF.query ~ O_MEFGCMA_WOTSTWES_NOPRF_SampleSample.query : 
    ={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, chad, m} ==> ={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, res}.
proof.
proc; inline *.
by sim; auto.
qed.

local equiv O_MEFGCMA_WOTSTWES_NOPRF_SampleSample_LoopSnocSample : 
  O_MEFGCMA_WOTSTWES_NOPRF_SampleSample.query ~ O_MEFGCMA_WOTSTWES_NOPRF_LoopSnocSample.query : 
    ={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, chad, m} ==> ={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, res}.
proof.
proc.
...
local equiv O_MEFGCMA_WOTSTWES_NOPRF_LoopSnocSample_Alt : 
  O_MEFGCMA_WOTSTWES_NOPRF_LoopSnocSample.query ~ O_MEFGCMA_WOTSTWES_NOPRF_Alt.query : 
    ={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, chad, m} ==> ={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, res}.
proof.
proc; inline *.
seq 5 3 : (={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, ad, m} /\ l{1} = sk{2}); last by sim => /#.
while (   #post
       /\ n{1} = len
       /\ i{1} = size sk{2} 
...
local equiv O_MEFGCMA_WOTSTWES_NOPRF_Orig_Alt : 
  O_MEFGCMA_WOTSTWES_NOPRF.query ~ O_MEFGCMA_WOTSTWES_NOPRF_Alt.query : 
    ={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, chad, m} ==> ={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, res}.
proof.
transitivity O_MEFGCMA_WOTSTWES_NOPRF_SampleSample.query 
             (={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, chad, m} ==> ={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, res}) 
             (={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, chad, m} ==> ={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, res}) => [/# | // | |].
+ by apply O_MEFGCMA_WOTSTWES_NOPRF_Orig_SampleSample.
transitivity O_MEFGCMA_WOTSTWES_NOPRF_LoopSnocSample.query 
             (={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, chad, m} ==> ={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, res}) 
             (={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, chad, m} ==> ={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, res}) => [/# | // | |].
+ by apply O_MEFGCMA_WOTSTWES_NOPRF_SampleSample_LoopSnocSample.
by apply O_MEFGCMA_WOTSTWES_NOPRF_LoopSnocSample_Alt.
qed.
...
*)  
local module O_Game34_WOTSTWES_SampleSample : Oracle_MEFGCMA_WOTSTWES = {
  include var O_MEFGCMA_WOTSTWES [-query]
...
local module O_Game34_WOTSTWES_LoopSnocSample : Oracle_MEFGCMA_WOTSTWES = {
  include var O_MEFGCMA_WOTSTWES [-query]
  ...
  proc query(chad : chadrs, m : msgWOTS) : pkWOTS * sigWOTS = {
    ...
    while (i < len) {
      ...
      if (em_ele = 0) { 
        ...
        sig_ele <- cf ps (set_chthidx ad i) (em_ele - 1) 1 (val sk_ele);
      }
...
local module O_Game34_WOTSTWES_LoopSnocSampleAlt : Oracle_MEFGCMA_WOTSTWES = {
  include var O_MEFGCMA_WOTSTWES [-query]
...
local module O_Game34_WOTSTWES_Alt : Oracle_MEFGCMA_WOTSTWES = {
  include var O_MEFGCMA_WOTSTWES [-query]
...
local equiv O_Game34_WOTSTWES_Orig_SampleSample : 
  O_Game34_WOTSTWES.query ~ O_Game34_WOTSTWES_SampleSample.query : 
    ={glob O_MEFGCMA_WOTSTWES, chad, m} ==> ={glob O_MEFGCMA_WOTSTWES, res}.
proof.
proc; inline *.
by sim; auto.
qed.
...
+ by sp; call Sample_LoopSnoc_eq; skip.
sp; sim.
while (   ={O_MEFGCMA_WOTSTWES.ps, ad, m, sk, sig, em}
       /\ size sig{1} = i{2}
       /\ 0 <= i{2} <= len
       /\ 0 <= size sig{1} <= len).
+ auto => />; smt(size_rcons).
...
+ sp; wp => /=.
  while (   ={O_MEFGCMA_WOTSTWES.ps, ad, m}
         /\ n{1} = len
         /\ l{1} = sk{2}
         /\ i0{1} = size sk{2}
         /\ 0 <= i0{1} <= len
  ...
  by auto => />; smt(ge2_len).
sim => /=.
while (   ={O_MEFGCMA_WOTSTWES.ps, ad, m, sk, em, sig}
       /\ i{1} = size sig{2}
       /\ 0 <= i{1} <= len
       /\ 0 <= size sig{2} <= len).
...
seq 5 5 : (   #pre 
           ...
           /\ (forall (i : int), 0 <= i < len =>
                 ...
                 then cf O_MEFGCMA_WOTSTWES.ps{2} (set_chthidx ad{2} i) (BaseW.val em{2}.[i] - 1) 1 (val (nth witness sk{2} i))
                 else nth witness sk{2} i)
           /\ size sig{2} = len) => /=.
+ swap{1} [4..5] -1; swap{2} 3 -1; sp; wp => /=.
  while (   ={O_MEFGCMA_WOTSTWES.ps, ad, m, sk, em}
         /\ is_chtype ad{1}
         /\ sig{1} = [] 
  ...
  by skip => />; smt(ge2_len ChainingAddress.valP).
sim => /=.
while{1} (   ={O_MEFGCMA_WOTSTWES.ps, ad, m, sk, em}
          /\ (forall (i : int),
               0 <= i && i < len =>
               nth witness sig{2} i =
               if BaseW.val em{2}.[i] <> 0
...
(* 
  ...
  Identical to M_EF_GCMA_WOTSTWES, only differing in the query procedure of the 
  provided Oracle_MEFGCMA_WOTSTWES (see above for the different oracle implementations). 
*)
local module Game0_WOTSTWES = {
  include M_EF_GCMA_WOTSTWES(A, O_Game0_WOTSTWES, O_THFC_Default)
}.
 
(* 
  Game2_WOTSTWES; third game in game sequence.
  Identical to M_EF_GCMA_WOTSTWES, only differing in the 
  query procedure of the provided Oracle_MEFGCMA_WOTSTWES (see above for the 
  different oracle implementations).
*)
local module Game2_WOTSTWES = {
  include M_EF_GCMA_WOTSTWES(A, O_Game2_WOTSTWES, O_THFC_Default)
}.

(* 
  Game3_WOTSTWES; fourth game in game sequence.
  Identical to M_EF_GCMA_WOTSTWES (and hence to Game2_WOTSTWES), only differing in the 
  query procedure of the provided Oracle_MEFGCMA_WOTSTWES (see above for the 
  different oracle implementations).
*)
local module Game3_WOTSTWES = {
  include M_EF_GCMA_WOTSTWES(A, O_Game34_WOTSTWES, O_THFC_Default)
}.
...
local module Game4_WOTSTWES = {
  ...
  proc main() : bool = {
    ...
    return 0 <= nrqs <= d /\ 0 <= i < nrqs /\ 
           is_valid /\ is_fresh /\ dist_preffour /\ disj_preffour adlO adlOC /\ !hchwcoll;
  }
}.
...
seq 9 9 : (={glob A, glob O_MEFGCMA_WOTSTWES, glob O_THFC_Default, ps}); last by sim.
sp => /=.
call (: ={glob O_MEFGCMA_WOTSTWES, glob O_THFC_Default}); last by auto.
+ conseq (: ={glob O_MEFGCMA_WOTSTWES, chad, m} ==> ={glob O_MEFGCMA_WOTSTWES, res}) => //.
...
(* 
  Reduction adversary that reduces from PRF for prf_sk to distinguishing between
  Game0 and MEFGCMA_WOTSTWES without PRF
*)
local module (R_PRF_Game0NOPRFWOTSTWES : Adv_PRF) (O : Oracle_PRF) = {
  module O_R_PRF_Game0NOPRFWOTSTWES : Oracle_MEFGCMA_WOTSTWES = {
    include var O_MEFGCMA_WOTSTWES [-query]
...
(* 
  ...
  Game3_WOTSTWES and Game4_WOTSTWES 
*)
local module (R_SMDTTCRC_Game34WOTSTWES : Adv_SMDTTCRC) (O : Oracle_SMDTTCR, OC : Oracle_THFC) = {
  module O_R_SMDTTCRC_Game34WOTSTWES : Oracle_MEFGCMA_WOTSTWES = {
    include var O_MEFGCMA_WOTSTWES [-init, query]
    var xll : dgstblock list list
...
(*
(* Reduction adversary that reduces from SM_DT_PRE_C (for f and thfc) to Game4_WOTSTWES *)
local module (R_SMDTPREC_Game4WOTSTWES : Adv_SMDTPREC) (O : Oracle_SMDTPRE, OC : Oracle_THFC) = {
  module O_R_SMDTPREC_Game4WOTSTWES : Oracle_MEFGCMA_WOTSTWES = {
    include var O_MEFGCMA_WOTSTWES [-query, init]
    var adl : adrs list
  ...
  module O_R_SMDTUDC_DistRCHil : Oracle_DistRCH = {
    ...
    proc query(chad : chadrs, m : msgWOTS) : dgstblock list = {
      ...
      while (size chal' < len) {
        ...
        if (i < em_ele - 1) {
          chal_ele' <@ O_SMDTUD_Default.query(set_htbidx (set_chthidx ad (size chal')) i);
        } else {
          chal_ele' <$ ddgstblock;
        } 
      ...
      while (size chal < len) {
        ...
        if (i < em_ele - 1) {
          ...
          j <- 1;
          while (j < em_ele - 1 - i) {
            chal_ele <@ O_THFC_Default.query(set_htbidx (set_chthidx ad (size chal)) (i + j), val chal_ele);
            j <- j + 1;
          }
        } else {
          chal_ele <- nth witness chal' (size chal);
  ...
  module O_DistRCHi : Oracle_DistRCH = {
    ...
    proc query(chad : chadrs, m : msgWOTS) : dgstblock list = {
      ...
      while (size chal < len){
        chal_ele' <- nth witness chal' (size chal);
        em_ele <- BaseW.val em.[size chal];
        chal_ele <- cf ps (set_chthidx ad (size chal)) i (em_ele - 1 - i) (val chal_ele');
        chal <- rcons chal chal_ele;
      }
...
proof.
proc; inline *.
seq 18 18 : (={glob A, glob O_MEFGCMA_WOTSTWES, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, ps}); last by sim.
call (:   ={glob O_THFC_Default, glob O_MEFGCMA_WOTSTWES, glob O_DistRCH, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl} 
...
(* -- Game-Playing Proof -- *)
(* - Zeroeth step: Show equivalence between M_EF_GCMA_WOTSTWES and Game0_WOTSTWES - *)
local equiv MEFGCMA_Game0_WOTSTWES :
  M_EF_GCMA_WOTSTWES(A, O_MEFGCMA_WOTSTWES, O_THFC_Default).main ~ Game0_WOTSTWES.main : 
    ={glob A} ==> ={res}.
proof.
proc; inline *.
seq 11 11 : (={glob A, O_MEFGCMA_WOTSTWES.qs, O_THFC_Default.tws, ps}); last by sim.
call (: ={glob O_MEFGCMA_WOTSTWES, glob O_THFC_Default}); [|by proc; inline *; sim | by auto]. 
proc; inline *.
wp => /=.
while (   ={glob O_MEFGCMA_WOTSTWES, ad, em}
       /\ sig0{1} = sig{2} 
       /\ size sig0{1} <= len 
       /\ size sig{2} <= len
       /\ skWOTS0{1} = insubd sk{2}
       /\ size sk{2} = len
       /\ ps0{1} = O_MEFGCMA_WOTSTWES.ps{1}).
+ by wp; skip => |> &1 lelen_szsig ltlen_szsig; smt(size_rcons DBLL.insubdK).
wp => /=.
while{1} (   0 <= size skWOTS3{1} <= len
          /\ (forall i, 0 <= i < len => 
                nth witness skWOTS1{1} i = prf_sk ss2{1} (ps3{1}, (set_htbidx (set_chthidx ad2{1} i) 0)))
          /\ (forall i, 0 <= i < size skWOTS3{1} => nth witness skWOTS3{1} i = nth witness skWOTS1{1} i))
         (len - size skWOTS3{1}).
+ move => _ z.
  by wp; skip => |> *; smt(size_rcons nth_rcons).
wp => /=.
while (   ={glob O_MEFGCMA_WOTSTWES}
       /\ pkWOTS0{1} = pk{2} 
       /\ size pkWOTS0{1} <= len 
       /\ size pk{2} <= len
       /\ size sk{2} = len
       /\ skWOTS2{1} = insubd sk{2}
       /\ ps2{1} = O_MEFGCMA_WOTSTWES.ps{1}
       /\ ad1{1} = ad{2}).
+ by wp; skip => |> &1 lelen_szpk ltlen_szpk; smt(size_rcons DBLL.insubdK).
wp => /=.
while (   ={glob O_MEFGCMA_WOTSTWES}
       /\ 0 <= size skWOTS1{1} <= len
       /\ ss1{1} = O_MEFGCMA_WOTSTWES.ss{2}
       /\ ps1{1} = O_MEFGCMA_WOTSTWES.ps{1}
       /\ skWOTS1{1} = sk{2}
       /\ ad0{1} = ad{2}
       /\ (forall i, 0 <= i < size skWOTS1{1} =>
             nth witness skWOTS1{1} i = prf_sk ss1{1} (ps1{1}, (set_htbidx (set_chthidx ad0{1} i) 0)))).
...
(* - First step: Reduce PRF of prf_sk to distinguishing between Game0_WOTSTWES and- M_EF_GCMA_WOTSTWES without PRF *)
local lemma Step_Game0_MEFGCMA_WOTSTWES_NOPRF_PRF &m :
  `|Pr[Game0_WOTSTWES.main() @ &m : res] - Pr[M_EF_GCMA_WOTSTWES(A, O_MEFGCMA_WOTSTWES_NOPRF, O_THFC_Default).main() @ &m : res]|
  =
...
+ byequiv => //=.
  proc; inline *.
  wp => /=.
  seq 11 14 : (={glob A, O_MEFGCMA_WOTSTWES.qs, O_THFC_Default.tws, ps}); last by sim.
  call (:   ={glob O_THFC_Default, O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs} 
         /\ O_PRF_Default.b{2} = false
         /\ O_MEFGCMA_WOTSTWES.ss{1} = O_PRF_Default.k{2}); conseq => />.
  - proc. 
    inline O_PRF_Default.query.
    seq 3 3 : (={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, ad, m, sk}); last by sim.
    while (   #post 
           /\ O_PRF_Default.b{2} = false
           /\ O_MEFGCMA_WOTSTWES.ss{1} = O_PRF_Default.k{2}); last by auto.
    rcondf{2} 2; first by auto.     
    by wp; skip.
  - by sim. 
  by wp; rnd; wp; rnd; wp; skip.
byequiv => //=.
transitivity M_EF_GCMA_WOTSTWES(A, O_MEFGCMA_WOTSTWES_NOPRF_Alt, O_THFC_Default).main
             (={glob A} ==> ={res})
             (arg{2} = true /\ ={glob A} ==> ={res}) => [/# | // | |].             
+ proc.
  seq 5 5 : (={glob A, O_MEFGCMA_WOTSTWES.qs, O_THFC_Default.tws, ps}); last by sim.
  call (: ={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, O_THFC_Default.pp, O_THFC_Default.tws}) => />.
  - conseq (: ={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, chad, m} 
              ==> 
              ={O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs, res}) => //.
    * by apply O_MEFGCMA_WOTSTWES_NOPRF_Orig_Alt.  
  - by proc; sim.
...
conseq (: _ 
          ...
          /\ (dist_preffour{2}
             ...
             (   (0 <= nrqs{1} <= d /\ 0 <= i{1} < nrqs{1}
              /\ is_valid{1} /\ is_fresh{1} /\ disj_preffour adlO{1} adlOC{1})
              =
                 (0 <= nrqs{2} <= d /\ 0 <= i{2} < nrqs{2}
              /\ is_valid{2} /\ is_fresh{2} /\ disj_preffour adlO{2} adlOC{2})))) => [/#|].
inline{1} 13; inline{1} 12; inline{1} 11; inline{1} 10; inline{1} 7.
inline{2} 13; inline{2} 12; inline{2} 11; inline{2} 10; inline{2} 7.
seq 5 5 : ((uniq_preffour (map (fun (q : _ * _ * _ *_) => q.`1) O_MEFGCMA_WOTSTWES.qs{1})
           =
           uniq_preffour (map (fun (q : _ * _ * _ *_) => q.`1) O_MEFGCMA_WOTSTWES.qs{2}))
           /\
           (uniq_preffour (map (fun (q : _ * _ * _ *_) => q.`1) O_MEFGCMA_WOTSTWES.qs{2})
            =>
            ={glob A, O_MEFGCMA_WOTSTWES.qs, O_THFC_Default.tws, ps})).
+ call (: (! (uniq_preffour (map (fun (q : _ * _ * _ *_) => q.`1) O_MEFGCMA_WOTSTWES.qs))),
              ={glob O_THFC_Default, O_MEFGCMA_WOTSTWES.ps, O_MEFGCMA_WOTSTWES.qs}
           /\ O_PRF_Default.b{2} = true
           /\ O_MEFGCMA_WOTSTWES.ss{1} = O_PRF_Default.k{2}
           /\ all is_chtype (map (fun (q : _ * _ * _ * _) => q.`1) O_MEFGCMA_WOTSTWES.qs{1})
           /\ (forall (ad : adrs), 
                 (get_preffour ad \in (map (fun (q : _ * _ * _ *_) => get_preffour q.`1) O_MEFGCMA_WOTSTWES.qs{2})) 
                 =>
                 (exists (ad' : adrs), (O_MEFGCMA_WOTSTWES.ps{2}, ad') \in O_PRF_Default.m{2} /\ get_preffour ad = get_preffour ad'))
            /\ (forall (ad : adrs), 
                 (O_MEFGCMA_WOTSTWES.ps{2}, ad) \in O_PRF_Default.m{2}
                 =>
                 (get_preffour ad \in (map (fun (q : _ * _ * _ *_) => get_preffour q.`1) O_MEFGCMA_WOTSTWES.qs{2}))),
           (uniq_preffour (map (fun (q : _ * _ * _ *_) => q.`1) O_MEFGCMA_WOTSTWES.qs{1})
            =
            uniq_preffour (map (fun (q : _ * _ * _ *_) => q.`1) O_MEFGCMA_WOTSTWES.qs{2}))) => //=.
  - move=> O OC; apply (A_choose_ll O OC).
  - proc.
    sp 1 1; wp => /=.
    case (get_preffour ad{2} \in map (fun (q : _ * _ * _ *_) => get_preffour q.`1) O_MEFGCMA_WOTSTWES.qs{2}).
    * conseq (: _ ==> true) => />. 
      + move=> &2 uqpfqs qsch relqsm relmqs pfadin pk sig m pk' csig'.
        split => [nuqpfrcqs |]. 
        - apply negbTE => @/uniq_preffour.
          rewrite -map_comp /(\o) map_rcons /= rcons_uniq negb_and /=.
          left; move/mapP: pfadin => [q] [qin /= ->].
          by rewrite mapP; exists q.
        pose uqpfrcqs := uniq_preffour _; have //: ! uqpfrcqs. 
        rewrite /uqpfrcqs /uniq_preffour.
        rewrite -map_comp /(\o) map_rcons /= rcons_uniq negb_and /=.
        left; move/mapP: pfadin => [q] [qin /= ->].
        by rewrite mapP; exists q.
      ...
      + inline{2} 1.
        ...
        by sp; if{2}; auto. 
      by wp; skip. 
    while (={ad, sk, sig, em, O_MEFGCMA_WOTSTWES.ps} /\ size sk{1} = len /\ 0 <= size sig{1} <= len).
    * wp; skip => />; smt(size_rcons).
    wp.
    while (={ad, pk, sk, O_MEFGCMA_WOTSTWES.ps} /\ size sk{1} = len /\ 0 <= size pk{1} <= len).
    * wp; skip => />; smt(size_rcons).
    wp => /=.
    ...
    while (   ={ad, sk}
           /\ is_chtype ad{2}
           /\ O_PRF_Default.b{2} = true
           /\ ! (get_preffour ad{2} \in map (fun (q : adrs * msgWOTS * pkWOTS * sigWOTS) => get_preffour q.`1) O_MEFGCMA_WOTSTWES.qs{2})
           /\ (forall (ad' : adrs), 
                 (get_preffour ad' \in (map (fun (q : _ * _ * _ *_) => get_preffour q.`1) O_MEFGCMA_WOTSTWES.qs{2})) 
                 =>
                 (exists (ad'' : adrs), (O_MEFGCMA_WOTSTWES.ps{2}, ad'') \in m /\ get_preffour ad' = get_preffour ad''))
           /\ (forall (ad' : adrs),
                 (O_MEFGCMA_WOTSTWES.ps{2}, ad') \in m 
                 =>
                 (get_preffour ad' \in (map (fun (q : _ * _ * _ *_) => get_preffour q.`1) O_MEFGCMA_WOTSTWES.qs{2})))
           /\ (forall (ad' : adrs), 
                 (O_MEFGCMA_WOTSTWES.ps{2}, ad') \in O_PRF_Default.m{2} 
                 <=> 
                 (((O_MEFGCMA_WOTSTWES.ps{2}, ad') \in m) \/ (ad' \in mkseq (fun (i : int) => set_htbidx (set_chthidx ad{2} i) 0) (size sk{2}))))
           /\ 0 <= size sk{2} <= len).
    * inline *.
      rcondt{2} 2; first by auto.
      rcondt{2} 2.
      + auto => /> &1 adch npfadin relqsm relmqs relmm ge0_szsk _ ltlen_szsk.
        move/iffLR: (relmm (set_htbidx (set_chthidx ad{m0} (size sk{m0})) 0)) => /contra; apply.
        rewrite negb_or; split.
        - move: (relmqs (set_htbidx (set_chthidx ad{m0} (size sk{m0})) 0)) => /contra /(_ _) //.
          pose gpfss := get_preffour _; have -> //: gpfss = get_preffour ad{m0}; rewrite /gpfss.
          by rewrite eqpreffour_setchidx_sethidx //; smt(val_w).
        rewrite mapP negb_exists => i /=; rewrite negb_and -implybE mem_iota /= => rng_i.
        by apply setchidx_sethidx_neqchidx => //; smt(val_w).
    ...
    split; first by smt(mkseq0 ge2_len ChainingAddress.valP).
    ...
    split; first by rewrite map_rcons -cats1 all_cat /= qsch ChainingAddress.valP.
    split => [ad' pfadpin | ad' adpin]. 
    * move/mapP: pfadpin => [q] [] /= + ->; rewrite mem_rcons /= => -[-> /= | qin].
      + exists (set_htbidx (set_chthidx (ChainingAddress.val chad{2}) 0) 0) => //=.
        split; first by rewrite relmrm mapP; right; exists 0 => //=; smt(mem_iota ge2_len).
        by rewrite eq_sym eqpreffour_setchidx_sethidx //=; smt(ge2_len val_w ChainingAddress.valP).
      move: (relqsm q.`1 _); first by rewrite mapP; exists q.
      by move=> [ad''] [adppin ->]; exists ad''; rewrite relmrm adppin.
    rewrite map_rcons mem_rcons; case (get_preffour ad' = get_preffour (ChainingAddress.val chad{2})) => [-> //| neqgpf /=].
    ...
    by move: (eqpreffour_setchidx_sethidx (ChainingAddress.val chad{2}) i 0 _ _ _) neqgpf => //=; smt(ge2_len val_w ChainingAddress.valP).
  - move=> &2 @/uniq_preffour nuqpfqs.
    proc.
    wp => /=.
    conseq (: _ ==> true). 
    * move=> &1 [#] equqpfqs adch pk' sig' /=.
      by rewrite nuqpfqs /uniq_preffour 2!map_rcons rcons_uniq /= equqpfqs nuqpfqs.
  ...
  - move=> &1.
    ...
    wp => /=.
    conseq (: _ ==> true).
    * move => &2 [#] @/uniq_preffour nuqpfqs equqpfqs adch pk' sig' /=.
...
case (uniq_preffour (map (fun (q : _ * _ * _ *_) => q.`1) O_MEFGCMA_WOTSTWES.qs{2})).
+ sim => /> /#. 
wp => /=; conseq (: _ ==> true) => />.
inline *.
wp.
...
(* - Second step: Show equivalence between M_EF_GCMA_WOTSTWES without PRF and Game2_WOTSTWES - *)
local equiv MEFGCMA_WOTSTWES_NOPRF_Game2_WOTSTWES : 
  M_EF_GCMA_WOTSTWES(A, O_MEFGCMA_WOTSTWES_NOPRF, O_THFC_Default).main ~ Game2_WOTSTWES.main : 
    ={glob A} ==> ={res}.
proof.
proc; inline *.
seq 12 12 : (={glob O_MEFGCMA_WOTSTWES, glob O_THFC_Default, ps, i, m', sig'}); last by sim.
do 2! call (: ={glob O_MEFGCMA_WOTSTWES, glob O_THFC_Default}); [|by proc; inline *; sim | by auto].
proc.
...
while{1} (   is_chtype ad{1}
          /\ (forall (i : int), 0 <= i < size sig => 
               nth witness sig i 
               = 
               cf O_MEFGCMA_WOTSTWES.ps (set_chthidx ad i) 0 (BaseW.val em.[i]) (val (nth witness sk i))){1}
          /\ size sig{1} <= len)
         (len - size sig{1}).
+ move=> _ z.
...
while{1} (   is_chtype ad{1}
          /\ (forall (i : int), 0 <= i < size pk => 
                nth witness pk i
                = 
                cf O_MEFGCMA_WOTSTWES.ps (set_chthidx ad i) 0 (w - 1) (val (nth witness sk i))){1}
          /\ size pk{1} <= len)
         (len - size pk{1}).
+ move=> _ z.
...
while{2} (    is_chtype ad{2}
          /\ (forall (i : int), 0 <= i < size pk => 
                nth witness pk i 
                =
                cf O_MEFGCMA_WOTSTWES.ps (set_chthidx ad i) 0 (w - 1) (val (nth witness sk i))){2}
          /\ size pk{2} <= len)
         (len - size pk{2}).
+ move=> _ z.
  wp; skip => |> &m adch valpkele leln_szpk ltlen_szspk.
...
while{2} (   is_chtype ad{2}
          /\ (forall (i : int), 0 <= i < size sig => 
                nth witness sig i 
                = 
                cf O_MEFGCMA_WOTSTWES.ps (set_chthidx ad i) 0 (BaseW.val em.[i]) (val (nth witness sk i))){2}
          /\ size sig{2} <= len)
         (len - size sig{2}).
+ move=> _ z.
  wp; skip => |> &m adch valsigele leln_szsig ltlen_szsig.
...
+ byequiv => //.
  proc; inline *.
  seq 10 16 : (   ={glob A, glob O_THFC_Default, O_MEFGCMA_WOTSTWES.qs, ps}
              /\ b{2} = false
              /\ (O_MEFGCMA_WOTSTWES.ps = ps){1}
              /\ (O_DistRCH.b = b){2}
              /\ (O_DistRCH.ps = ps){2}
              /\ (O_THFC_Default.pp = ps){1}
              /\ (O_THFC_Default.pp = ps){2}
              /\ O_MEFGCMA_WOTSTWES.qs{1} = []
              /\ O_MEFGCMA_WOTSTWES.qs{2} = []
              /\ O_THFC_Default.tws{1} = []
              /\ O_THFC_Default.tws{2} = []
              /\ R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl{2} = []).
  - by auto => />.
  seq 1 1 : (   ={glob A, O_MEFGCMA_WOTSTWES.qs, ps}
             /\ O_THFC_Default.tws{1} = R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl{2}).
  - call (:   ={O_MEFGCMA_WOTSTWES.qs, O_THFC_Default.pp} 
           /\ O_DistRCH.b{2} = false
           /\ O_MEFGCMA_WOTSTWES.ps{1} = O_DistRCH.ps{2}
           /\ O_MEFGCMA_WOTSTWES.ps{1} = O_THFC_Default.pp{2}
           /\ O_THFC_Default.tws{1} = R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl{2}).
    * proc; inline *.
      ...
      seq 6 12 : (   #pre
                  ...
                  /\ (forall (i : int), 0 <= i < len =>
                       nth witness sig{1} i 
                       = 
                       cf O_MEFGCMA_WOTSTWES.ps{1} (set_chthidx ad{1} i) 0 (BaseW.val em{1}.[i]) (val (nth witness sk{1} i)))
                  /\ (forall (i : int), 0 <= i < len => 
                       nth witness chal{2} i
                       = 
                       cf O_DistRCH.ps{2} (set_chthidx ad{2} i) 0 (BaseW.val em{2}.[i] - 1) (val (nth witness chal'{2} i)))).
      + wp => /=.
        while (   ={m, ad}
               /\ ad0{2} = ad{2}
               /\ is_chtype ad{1}
               ...
               /\ (forall (i : int), 0 <= i < size chal0{2} => 
                    ...
                    = 
                    cf O_DistRCH.ps{2} (set_chthidx ad0{2} i) 0 (BaseW.val em0{2}.[i] - 1) (val (nth witness chal'{2} i)))); last first.
        - by wp; rnd; wp; skip => />; smt(ge2_len ChainingAddress.valP).
      ...
      while{2} (   ={m, ad, pk, em}
                ...
                /\ (sig_ele{2} 
                    = 
                    if em_ele{2} <> 0 
                    then cf O_THFC_Default.pp{2} (set_chthidx ad{2} (size pk{2})) (em_ele{2} - 1) 1 (val chal_ele{2})
                    else chal_ele{2})
                /\ pk_ele{2} 
                   = 
                   cf O_THFC_Default.pp{2} (set_chthidx ad{2} (size pk{2})) em_ele{2} j{2} (val sig_ele{2}))
               (w - 1 - em_ele{2} - j{2}).
      + move=> &1 z.
        ...
        by rewrite -/f eq_sym /cf chS 1:ischtype_setchidx 3:valP //; smt(BaseW.valP).
      ...
      + split => [| j]; first rewrite ch0 1:ischtype_setchidx 3:valP // valKd; smt(BaseW.valP).
        ...
        - rewrite !andbA; split; first by smt(size_rcons).
          ...
          by rewrite /cf ?ch0 1,4:ischtype_setchidx // 1,2:valP.
        congr; rewrite valchal 1:// eq0_em /=; do 2! congr.
        by rewrite /cf ch0 1:ischtype_setchidx // 1:valP // valKd.
      rewrite valP -/f.
      split => [@/cf | jr]. 
      + rewrite ch1 1:ischtype_setchidx // 1:valP //=; first 2 by smt(BaseW.valP).
        by rewrite ch0 1:ischtype_setchidx // 1:valP //= valKd; smt(BaseW.valP).         
      ...
      + rewrite size_rcons => i ge0_i ltszsig1_i; rewrite nth_rcons.
        ...
        by rewrite (valsig (size sig{2})) 2:(valchal (size sig{2})) 1,2:/# /cf chS; smt(BaseW.valP DigestBlock.valP ischtype_setchidx).
      rewrite eq_sym (valchal (size pk{2})) 1:/#.
      have {1}->: BaseW.val em{2}.[size pk{2}] - 1 = 0 + (BaseW.val em{2}.[size pk{2}]) - 1 by trivial.
      rewrite /cf -chS 1:ischtype_setchidx // 1:valP //; first 2 by smt(BaseW.valP).
      rewrite eq_sym.
      have {1}->:
        w - 1
        =
        BaseW.val em{2}.[size pk{2}] + (w - 1 - BaseW.val em{2}.[size pk{2}]) by smt().
      by rewrite /cf ch_comp //; smt(BaseW.valP DigestBlock.valP ischtype_setchidx).
    * proc; inline *. 
      by auto.
    by skip.
  wp => /=.
  by sim : (   ={m, m', i, O_MEFGCMA_WOTSTWES.qs}
            /\ pkWOTS0{1} = pkWOTS{2}
            /\ pkWOTS1{1} = pkWOTS0{2}
            /\ O_THFC_Default.tws{1} = R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl{2}).
byequiv => //.
  ...
  - proc; inline *.
    ...
    * by auto => />; rewrite ChainingAddress.valP.
    seq 2 0 : (   #pre
               ...
               /\ (forall (i : int), 0 <= i < len =>
                    ...
                    then cf O_MEFGCMA_WOTSTWES.ps{1} (set_chthidx ad{1} i) (BaseW.val em{1}.[i] - 1) 1 (val (nth witness sk{1} i))
    ...
    * skip => /> &1 &2 adch chin eqlen_szsig valsig. 
    ...
    while{2} (   ={m, ad, pk, em}
              ...
              /\ pk_ele{2} = cf O_THFC_Default.pp{2} (set_chthidx ad{2} (size pk{2})) em_ele{2} j{2} (val sig_ele{2}))
             (w - 1 - em_ele{2} - j{2}).
      + move=> &1 z.
      ...
      split => [@/cf | jr].
      + rewrite valP ch1 1:ischtype_setchidx // 1:valP //=; first 2 by smt(BaseW.valP).
        by rewrite ch0 1:ischtype_setchidx // 1:valP //= valKd; smt(BaseW.valP).
      split => [/# | /lezNgt gew1em_jr ltlen_szsig ge0_jr lew1em_jr fcf1].
      have ->: jr = w - 1 - BaseW.val em{2}.[size pk{2}] by smt().
      rewrite !andbA -andbA; split; last by smt(size_rcons). 
      ...
      + rewrite size_rcons => i ge0_i ltszsig1_i; rewrite nth_rcons.
        case (i < size sig{2}) => ?; first by rewrite (relsigs i).
        rewrite (: i = size sig{2}) 1:/# eq_sz /=.
        by rewrite (valsig (size sig{2})) 1:/# -eq_sz neq0_em /= /cf ch1 //; smt(BaseW.valP DigestBlock.valP ischtype_setchidx).
      by congr; rewrite valsig 1:/# neq0_em /cf ch1 1:ischtype_setchidx 3,6:valP //=; first 2 by smt(BaseW.valP).
    * proc; inline *. 
    by auto.
  by skip.
wp => /=.
...
(* 
  Auxiliary lemma.
  Reduce SM_DT_UD_C to distinguishing between two relevant distributions.
*)
...
+ congr; last congr.
  - byequiv => //.
    ...
    * by apply DistRCHi_Orig_Alt.
    proc; inline *. 
    seq 16 17: (   ={glob A, glob O_MEFGCMA_WOTSTWES, glob O_THFC_Default, O_DistRCH.ps, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, ps}
                /\ b{1} = false
                /\ i{2} = 0
                /\ O_DistRCH.b{1} = b{1}
    ...
    seq 1 1 : (={glob A, O_MEFGCMA_WOTSTWES.qs, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, ps}); last by sim.
    call (:   ={O_MEFGCMA_WOTSTWES.qs, O_THFC_Default.pp, O_DistRCH.ps, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl}
           /\ O_DistRCH.b{1} = false
  ...
  * by apply DistRCHi_Orig_Alt.
  ...
  - move=> &1 z.
    ...
    * by rewrite (valchal i) 1:ge0_i 1:ltsz_i.
    ...
    by rewrite /cf ch0 1:ischtype_setchidx 3:valP // valKd.
  wp; rnd; wp; skip => /> &1 chal chalin.
  split => [| chal']; first by smt(ge2_len ChainingAddress.valP).
...
+ rewrite 2!BRA.big_int &(BRA.eq_bigr) => k [ge0_k ltw1_k] /=.
  ...
  - byequiv => //.
    ...
    seq 6 8 : (   ={glob A, glob O_MEFGCMA_WOTSTWES, glob O_THFC_Default, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, O_DistRCH.b, i}
               /\ i{1} = k
               /\ b{2} = false
               /\ ps{1} = pp{2}
    ...
    * by inline *; auto.
    seq 1 2 : (   ={glob A, glob O_MEFGCMA_WOTSTWES, O_THFC_Default.pp, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, ps}
               /\ all is_chtype (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) O_MEFGCMA_WOTSTWES.qs{1})
               /\ O_SMDTUD_Default.ts{2} = relcqsad_udi O_MEFGCMA_WOTSTWES.qs{1} k
               /\ all (fun (ad : adrs) => 
                         ad \in R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl{2} 
                         \/ ad \in reltqsad_udi O_MEFGCMA_WOTSTWES.qs{1} k) 
                         O_THFC_Default.tws{2}).
    * wp => /=.
      call (:   ={glob O_MEFGCMA_WOTSTWES, O_THFC_Default.pp, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, O_DistRCH.b}
      ...
      + proc.
        seq 2 2 : (   ={glob O_MEFGCMA_WOTSTWES, O_THFC_Default.pp, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, O_DistRCH.b, ad, m, chal}
                   /\ is_chtype ad{1}
                   /\ O_DistRCH.ps{1} = O_SMDTUD_Default.pp{2}
                   /\ O_SMDTUD_Default.pp{2} = O_THFC_Default.pp{2}
                   /\ O_SMDTUD_Default.b{2} = false
                   ...
                   /\ R_SMDTUDC_DistRCHil.O_R_SMDTUDC_DistRCHil.i{2} = k
                   /\ all is_chtype (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) O_MEFGCMA_WOTSTWES.qs{1})
                   /\ O_SMDTUD_Default.ts{2} = relcqsad_udi (rcons O_MEFGCMA_WOTSTWES.qs{1} (ad{2}, m{2}, insubd pk{2}, insubd sig{2})) k
                   /\ all (fun (ad' : adrs) => 
                               ad' \in R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl{2} 
                               \/ ad' \in reltqsad_udi (rcons O_MEFGCMA_WOTSTWES.qs{1} (ad{2}, m{2}, insubd pk{2}, insubd sig{2})) k) 
                               O_THFC_Default.tws{2}
                   /\ size chal{1} = len).
        - inline *; swap{1} 7 -2.
          wp => /=.
          while (   ={glob O_MEFGCMA_WOTSTWES, O_THFC_Default.pp, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, O_DistRCH.b, ad, m, ad0, m0, em0, chal0}
                 /\ ad0{1} = ad{1}
                 /\ m0{1} = m{1}
                 ...
                 /\ (forall (i : int), 0 <= i < len =>
                       ...
                       then cf O_SMDTUD_Default.pp{2} (set_chthidx ad0{2} i) R_SMDTUDC_DistRCHil.O_R_SMDTUDC_DistRCHil.i{2} 1 (val (nth witness chal'{1} i)) 
          ...
          * swap{1} 1 1; sp 1 1; wp => /=.
            ...
            + wp; skip => /> &1 &2 *. 
              ...
              by rewrite /cf ch0 1:ischtype_setchidx 3:valP // valKd /#.
            while{2} (   is_chtype ad0{2} 
                      /\ R_SMDTUDC_DistRCHil.O_R_SMDTUDC_DistRCHil.i{2} = k
                      /\ ad0{2} = ad{2}
                      /\ m0{2} = m{2}
                      /\ em0{2} = encode_msgWOTS m0{2}
                      /\ em_ele0{2} = BaseW.val em0{2}.[size chal0{2}]
                      /\ chal_ele0{2} = cf O_THFC_Default.pp{2} (set_chthidx ad0{2} (size chal0{2})) (R_SMDTUDC_DistRCHil.O_R_SMDTUDC_DistRCHil.i{2} + 1) (j0{2} - 1) (val (nth witness chal'{2} (size chal0{2})))
                      /\ all (fun (ad' : adrs) => 
                             ad' \in R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl{2} 
                             \/ ad' \in reltqsad_udi (rcons O_MEFGCMA_WOTSTWES.qs{1} (ad{2}, m{2}, insubd pk{2}, insubd sig{2})) k) 
                             O_THFC_Default.tws{2}
            ...
            + auto => /> &2 adch twssp eqlen_szcp ge0_szc0 ltlen_szc0 ge1_j leem1k_j ltem1k_j.
              ...
              split. 
              - by rewrite valP -/f /cf eq_sym chS 1:ischtype_setchidx 3:valP // 1,2:/#; smt(BaseW.valP).
            ...
            split => [| twsr jr].
            + by rewrite /cf ch0 // 1:ischtype_setchidx 3:valP // valKd -eqsz eqlen_szcp /#.
            ...
            by rewrite ch_comp 1:ischtype_setchidx 3:valP //; smt(BaseW.valP).
          wp => /=.
          while (   ={glob O_MEFGCMA_WOTSTWES, O_THFC_Default.pp, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, O_DistRCH.b, ad, m, ad0, m0, em0}
                     /\ ad0{1} = ad{1}
                     /\ m0{1} = m{1}
                     /\ em0{1} = encode_msgWOTS m0{1}
                     /\ is_chtype ad{1}
                     ...
                     /\ O_SMDTUD_Default.ts{2} 
                        = 
                        relcqsad_udi O_MEFGCMA_WOTSTWES.qs{1} k 
          ...
          * wp; sp => /=.
            ...
            + move=> i ge0_i; rewrite size_rcons => ltszcp1_i.
              rewrite ?nth_rcons ?eqsz; case (i < size chal'{2}) => [ltszcp_i /# | /lezNgt geszcp_i].
              by rewrite (: i = size chal'{2}) 1:/# ltem1_k /= /cf ch1 1:ischtype_setchidx 3:valP // /#. 
          ...
          + rewrite 2!andbA; split; last by smt(ge2_len).
            rewrite -andbA; split; first by rewrite ChainingAddress.valP.
        ...
        - inline O_THFC_Default.query.
          ...
          while (   #pre
                 /\ ={j, sig_ele, pk_ele}
                 /\ pk_ele{1} = cf O_THFC_Default.pp{1} (set_chthidx ad{1} (size pk{1})) em_ele{1} j{1} (val sig_ele{1})
                 /\ 0 <= j{1} <= w - 1 - em_ele{1}).
          * wp; skip => /> &2 adch qsch twsrcsp eqlen_szc eqsz ge0_szpk lelen_szpk ltlen_szpk ge0_j lew1em_j ltw1em_j.
            ...
            + by rewrite ch0 1:ischtype_setchidx 3:valP // ch1 1:ischtype_setchidx 3:valP // 3:valKd; first 2 smt(BaseW.valP).
            by rewrite (chS _ _ _ _ (j{2} + 1)) 1:ischtype_setchidx 3:valP // 4:addrA //=; smt(BaseW.valP).
          ...
          * rewrite !andbA -andbA; split; last first.
            + by rewrite valP /cf ch0 1:ischtype_setchidx 3:valP // valKd; smt(BaseW.valP).
    ...
    * apply (IntOrder.ler_trans (size O_MEFGCMA_WOTSTWES.qs{2} * len)); first by smt(relcqsadudi_rng).
      by rewrite ler_pmul //; smt(ge2_len).
    apply (djl_parts _ _ R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl{2} (reltqsad_udi O_MEFGCMA_WOTSTWES.qs{2} k)) => //. 
    * by apply disj_relcqsadudi => /#.
    by apply disj_relcqsadudi_reltqsadudi => /#.
  byequiv => //.
  proc => /=; inline{1} 6.
  wp; inline{1} 5; inline{2} 5; inline{2} 6; inline{2} 4; inline{2} 7.
  seq 6 8 : (   ={glob A, glob O_MEFGCMA_WOTSTWES, glob O_THFC_Default, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, O_DistRCH.b}
             /\ i{1} = k + 1
             /\ i{2} = k
             /\ b{2} = true
             /\ ps{1} = pp{2}
             ...
             /\ R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl{1} = []).
  * by inline *; auto.
  seq 1 2 : (   ={glob A, glob O_MEFGCMA_WOTSTWES, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, ps}
             /\ all is_chtype (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) O_MEFGCMA_WOTSTWES.qs{1})
             /\ O_SMDTUD_Default.ts{2} = relcqsad_udi O_MEFGCMA_WOTSTWES.qs{1} k
             /\ all (fun (ad : adrs) => 
                       ad \in R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl{2} \/ ad \in reltqsad_udi O_MEFGCMA_WOTSTWES.qs{1} k) 
                       O_THFC_Default.tws{2}).
  * wp => /=.
    call (:   ={glob O_MEFGCMA_WOTSTWES, O_THFC_Default.pp, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, O_DistRCH.b}
           /\ O_DistRCH.ps{1} = O_SMDTUD_Default.pp{2}
    ...
    + proc.
      ...
      - inline *; swap{1} 5 -2.
        wp => /=.
        while (   ={glob O_MEFGCMA_WOTSTWES, O_THFC_Default.pp, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, O_DistRCH.b, ad, m, ad0, m0, em0, chal0, chal'}
               /\ ad0{1} = ad{1}
               /\ m0{1} = m{1}
               /\ em0{1} = encode_msgWOTS m0{1}
               /\ is_chtype ad{1}
        ...
        * swap{1} 1 1; sp 1 1; wp => /=.
          ...
          + auto => /> &2 adch twssp eqlen_szcp ge0_szc0 ltlen_szc0 ge1_j leem1k_j ltem1k_j.
            ...
            split.
            - by rewrite -/f /cf eq_sym chS // 1:ischtype_setchidx 3:valP //; smt(BaseW.valP).
            rewrite allP => ad'; rewrite mem_rcons /= => -[-> | adpin] /=.
            - right; rewrite /reltqsad_udi -flatten_mapP.
              exists (ad{2}, m{2}, insubd pk{2}, insubd sig{2}) => /=; split; first by rewrite mem_rcons.
              rewrite -flatten_mapP; exists (size chal0{2}) => /=; split; first by smt(size_iota mem_iota).
          ...
          by split => [| twsr jr]; [rewrite /cf ch0 1:ischtype_setchidx 3:valP //= valKd /# | smt(size_rcons)].
        wp => /=.
        while (   ={glob O_MEFGCMA_WOTSTWES, O_THFC_Default.pp, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, O_DistRCH.b, ad, m, ad0, m0, chal'}
                   /\ ad0{1} = ad{1}
                   /\ m0{1} = m{1}
                   /\ em0{2} = encode_msgWOTS m0{2}
                   /\ is_chtype ad{1}
        ...
        * wp; sp => /=.
        ...
        + rewrite ChainingAddress.valP /= andbA; split; last by smt(ge2_len).
          rewrite /relcqsad_udi_outer mkseq0 flatten_nil cats0 //=.
          rewrite allP => ad' adpin /=.
          move/allP: twssp => /(_ ad' adpin) /= [-> //|].
          move/flatten_mapP => [q] [qin /= adpinrel].
      ...
      - inline O_THFC_Default.query.
        ...
        * wp; skip => /> &2 adch qsch twsrcsp eqlen_szc eqsz ge0_szpk lelen_szpk ltlen_szpk  ge0_j lew1em_j ltw1em_j.
          ...
          + rewrite /cf ch0 1:ischtype_setchidx 3:valP // valKd.
            by rewrite -/f ch1 1:ischtype_setchidx 3:valP //; smt(ischtype_setchidx BaseW.valP).
          rewrite -/f /cf (chS _ _ _ _ (j{2} + 1)) 1:ischtype_setchidx 3:valP //; smt(BaseW.valP).
        wp; skip => /> &2 adch qsch twsrcsp eqlen_szc eqsz ge0_szpk lelen_szpk ltlen_szpk.
        split => [eq0_em | neq0_em *].
        * split => [| twsr jr /lezNgt gew1em_jr _ twsrrcsp ge0_jr lew1em_jr].  
          + by rewrite /cf ch0 1:ischtype_setchidx 3:valP //= valKd; split => [| /#].
          rewrite !andbA -5!andbA; split; last by smt(size_rcons).
          split; first by rewrite (relcqsadudi_rcons_qs _ _ _ _ (DBLL.insubd (rcons _ _)) (insubd pk{2}) _ (insubd sig{2})).
          rewrite allP=> adrs adrsin /=; move/allP: twsrrcsp => /(_ adrs adrsin) /= [-> // | ?].
          by rewrite (reltqsadudi_rcons_qs _ _ _ _ _ (insubd pk{2}) _ (insubd sig{2})); right.
...
+ rewrite eq_sym -(addr0 (BRA.bigi _ _ _ _)); congr; last first.
  ...
  seq 10 6 : (   ={glob A, pp, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, O_MEFGCMA_WOTSTWES.qs, O_SMDTUD_Default.ts, O_THFC_Default.tws}
               /\ i{1} = R_SMDTUDC_DistRCH.O_R_SMDTUDC_DistRCH.i{2}); last by wp => /=; sim.
  inline *.
  call (:   ={R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, O_MEFGCMA_WOTSTWES.qs, O_SMDTUD_Default.b, O_SMDTUD_Default.pp, O_SMDTUD_Default.ts, O_THFC_Default.tws, O_THFC_Default.pp}
         /\ R_SMDTUDC_DistRCHil.O_R_SMDTUDC_DistRCHil.i{1} = R_SMDTUDC_DistRCH.O_R_SMDTUDC_DistRCH.i{2}).
  - proc.
    inline *.
    seq 7 7 : (   ={m, em0, chal', ad0, ad, R_DistRCH_Game23WOTSTW.O_R_DistRCH_Game23WOTSTW_THFC.adl, O_MEFGCMA_WOTSTWES.qs, O_SMDTUD_Default.b, O_SMDTUD_Default.pp, O_SMDTUD_Default.ts, O_THFC_Default.tws, O_THFC_Default.pp}
               /\ is_chtype ad{1}
               /\ ad0{1} = ad{1}
    ...
    * wp; sp.
    ...
    by wp; skip => />; smt(ge2_len ChainingAddress.valP).
  - by proc; sim.
  by wp; rnd; rnd; skip.
rewrite eq_sym -(addr0 (BRA.bigi _ _ _ _)); congr; last first.
+ rewrite eq_sym.
...
+ proc.
  ...
  * wp; sp.
  ...
  wp; skip => />; smt(ge2_len ChainingAddress.valP).
+ by proc; sim.
by wp; rnd; rnd; skip.
qed.
...
(* - Third step: Reduce/Bound distinguishing between Game3_WOTSTWES and Game4_WOTSTWES - *)
...
+ byequiv => //. 
  proc; inline *.
  seq 12 11 : (={glob O_THFC_Default, O_MEFGCMA_WOTSTWES.qs, ps, i, m', sig'}); last first.
  - by wp; sim : (={i, m, m', O_MEFGCMA_WOTSTWES.qs, O_THFC_Default.tws} /\ pkWOTS0{1} = pkWOTS{2} /\ pkWOTS1{1} = pkWOTS0{2}).
  do 2! call (: ={glob O_THFC_Default, O_MEFGCMA_WOTSTWES.qs, O_MEFGCMA_WOTSTWES.ps}); first 2 by proc; inline *; sim.
   by auto.
...
byequiv => //.
proc; inline *. wp.
by wp; sim :  (={i, ps, pkWOTS0, pkWOTS, ad, m, m', sig, sig', O_MEFGCMA_WOTSTWES.qs, O_THFC_Default.tws}).
qed.


(* - Fourth step: Reduce SM-DT-TCR-C of f to distinguishing between Game3_WOTSTWES and Game4_WOTSTWES - *)
  ...
  all (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => 
         all (fun (j : int) =>
           ...
           let sig_ele = nth witness (val admpksig.`4) j in 
             ...
             = 
             cf ps (set_chthidx ad j) em_ele (w - 1 - em_ele) (val sig_ele)) (range 0 i)) qs.
         ...
         let (admpksig, xl) = (admpksigxl.`1, admpksigxl.`2) in
           all (fun (j : int) =>
             ...
             let sig_ele = nth witness (val admpksig.`4) j in 
               ...
               = 
               if em_ele <> 0
               then cf ps (set_chthidx ad j) (em_ele - 1) 1 (val (nth witness xl j))
               else nth witness xl j) (range 0 i)) (zip qs xll).
seq 1 1 : (    ={glob A, glob O_THFC_Default, O_MEFGCMA_WOTSTWES.qs, ps}
            /\ pp{2} = ps{2}
            /\ O_MEFGCMA_WOTSTWES.ps{1} = ps{1}
            /\ O_THFC_Default.pp{1} = ps{1}
            /\ O_SMDTTCR_Default.pp{2} = ps{2}
            /\ O_THFC_Default.pp{2} = ps{2}
            /\ size O_MEFGCMA_WOTSTWES.qs{1} = size R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{2}
            /\ all is_chtype (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) O_MEFGCMA_WOTSTWES.qs{1})
            /\ pkvals O_MEFGCMA_WOTSTWES.qs{1} O_MEFGCMA_WOTSTWES.ps{1} len
            /\ sigvals O_MEFGCMA_WOTSTWES.qs{1} R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{2} O_MEFGCMA_WOTSTWES.ps{1} len
            /\ unzip1 O_SMDTTCR_Default.ts{2} = relcqsad_tcr O_MEFGCMA_WOTSTWES.qs{1}
            /\ unzip2 O_SMDTTCR_Default.ts{2} = relcqsdg_tcr O_MEFGCMA_WOTSTWES.qs{1} R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{2} O_MEFGCMA_WOTSTWES.ps{1}).
+ call (:   ={glob O_THFC_Default, O_MEFGCMA_WOTSTWES.qs}
         /\ O_MEFGCMA_WOTSTWES.ps{1} = O_THFC_Default.pp{1}
         ...
         /\ sigvals O_MEFGCMA_WOTSTWES.qs{1} R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{2} O_MEFGCMA_WOTSTWES.ps{1} len
         /\ unzip1 O_SMDTTCR_Default.ts{2} = relcqsad_tcr O_MEFGCMA_WOTSTWES.qs{1}
         /\ unzip2 O_SMDTTCR_Default.ts{2} = relcqsdg_tcr O_MEFGCMA_WOTSTWES.qs{1} R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{2} O_MEFGCMA_WOTSTWES.ps{1}); last by auto.
  - proc; inline * => //.
    seq 6 5 : (   #pre
               /\ ={ad, pk, em}
               /\ is_chtype ad{1}
    ...
    * wp => /=.
      ...
      + move=> _ z; wp; skip => /> &1 adch _ valsig ltlen_szsig.
        ...
        rewrite nth_rcons; case (i < size sig{1}) => [ltszsig_i /# | /lezNgt geszsig_i].
        by rewrite (: i = size sig{1}) 1:/# /= neq0_em.
      wp; rnd; wp; skip => /> *; split; smt(ge2_len ChainingAddress.valP).
    wp => //=.
    while (   ={glob O_THFC_Default, ad, m, pk, em, O_MEFGCMA_WOTSTWES.qs} 
           /\ O_MEFGCMA_WOTSTWES.ps{1} = O_THFC_Default.pp{1}
           /\ O_THFC_Default.pp{1} = O_THFC_Default.pp{2} 
           /\ O_THFC_Default.pp{2} = O_SMDTTCR_Default.pp{2}
           /\ size O_MEFGCMA_WOTSTWES.qs{1} = size R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{2}
           /\ unzip1 O_SMDTTCR_Default.ts{2} 
              = 
              relcqsad_tcr O_MEFGCMA_WOTSTWES.qs{1} 
              ++ relcqsad_tcr_outer ad{2} m{2} (size pk{2})
           /\ unzip2 O_SMDTTCR_Default.ts{2} 
              = 
              relcqsdg_tcr O_MEFGCMA_WOTSTWES.qs{1} R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{2} O_MEFGCMA_WOTSTWES.ps{1} 
              ++ relcqsdg_tcr_outer O_MEFGCMA_WOTSTWES.ps{1} ad{2} m{2} (insubd sig{1}) xl{2} (size pk{2})
           /\ is_chtype ad{1} 
           /\ em{1} = encode_msgWOTS m{1} 
           /\ em{2} = encode_msgWOTS m{2} 
           /\ sk{1} = xl{2} 
           ...
           /\ (forall (i : int), 0 <= i < size pk{1} =>
                nth witness pk{1} i 
                =
                cf O_MEFGCMA_WOTSTWES.ps{1} (set_chthidx ad{1} i) (BaseW.val em{1}.[i]) (w - 1 - BaseW.val em{1}.[i]) (val (nth witness sig{1} i)))
           /\ (forall (i : int), 0 <= i && i < size sig{2} =>
                 nth witness sig{1} i = nth witness sig{2} i)).
    * wp => /=.
      while{2} (   ={glob O_THFC_Default, ad, m, pk, em, O_MEFGCMA_WOTSTWES.qs} 
                /\ O_MEFGCMA_WOTSTWES.ps{1} = O_THFC_Default.pp{1} 
                /\ O_THFC_Default.pp{1} = O_THFC_Default.pp{2} 
                /\ O_THFC_Default.pp{2} = O_SMDTTCR_Default.pp{2}
                /\ size O_MEFGCMA_WOTSTWES.qs{1} = size R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{2}
                /\ unzip1 O_SMDTTCR_Default.ts{2} 
                   ...
                   relcqsad_tcr O_MEFGCMA_WOTSTWES.qs{1} 
                   ++ relcqsad_tcr_outer ad{2} m{2} (size pk{2})
                   ++ relcqsad_tcr_inner (set_chthidx ad{2} (size pk{2})) em_ele{2} j{2}
                /\ unzip2 O_SMDTTCR_Default.ts{2} 
                   = 
                   relcqsdg_tcr O_MEFGCMA_WOTSTWES.qs{1} R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{2} O_MEFGCMA_WOTSTWES.ps{1} 
                   ++ relcqsdg_tcr_outer O_MEFGCMA_WOTSTWES.ps{1} ad{2} m{2} (insubd sig{1}) xl{2} (size pk{2})
                   ++ relcqsdg_tcr_inner O_MEFGCMA_WOTSTWES.ps{1} (set_chthidx ad{2} (size pk{2})) em_ele{2} sig_ele{2} x{2} j{2}
                /\ is_chtype ad{1} 
                /\ em{1} = encode_msgWOTS m{1} 
                /\ em{2} = encode_msgWOTS m{2} 
                /\ sk{1} = xl{2} 
                ...
                /\ pk_ele{2} = cf O_SMDTTCR_Default.pp{2} (set_chthidx ad{2} (size pk{2})) (em_ele{2}) j{2} (val sig_ele{2})
      ...
      + move=> &1 z.
        ...
        rewrite !andbA -2!andbA; split => [| /#].
        split; last first.
        - rewrite /cf ch_comp 1:ischtype_setchidx 3:valP //; first 2 by smt(BaseW.valP).
          by rewrite /cf ch1 1:ischtype_setchidx 3:valP //; smt(BaseW.valP).
        ...
        rewrite neq0_em /= => ->.
        by congr; rewrite (Ring.IntID.addrC j{2} 1) /cf ch_comp 1:ischtype_setchidx 3:valP //; smt(BaseW.valP ge2_len).
      ...
      + split => [| ts j]; last split => [/# | /lezNgt gew1em_j rcqsadp rcqsdgp ltlen_szsig2 eq_nthsig ge0_j lew1em_j]. 
        - rewrite !andbA -3!andbA; split; last first.
          * by rewrite /cf ch0 1:ischtype_setchidx 3:valP //= valKd; smt(size_ge0 BaseW.valP).
      ...
      split => [| ts j]; last split => [/# | /lezNgt gew1em_j rcqsadp rcqsdgp ltlen_szsig2 eq_nthsig ge0_j lew1em_j].
      + rewrite !andbA -3!andbA; split; last first. 
        - rewrite /cf ch0 1:ischtype_setchidx 3:valP // -eq_sz ltlen_szpk2 /=. 
          rewrite valKd /=; split; last by smt(BaseW.valP). 
          rewrite (valsig1 (size pk{2}) _) 2:neq0_em /cf; first by smt(size_ge0).
          by rewrite /cf ch1 1:ischtype_setchidx 3:valP //; smt(size_ge0 BaseW.valP).
        ...
        * by rewrite /relcqsad_tcr_inner neq0_em /= /mkseq iota1 //= cats1.
        ...
        by rewrite /cf ch0 1:ischtype_setchidx 3:valP // insubdK 1:valP.
      rewrite !andbA -8!andbA; split; last first. 
      + rewrite 4!andbA; split; first by smt(size_rcons).
        rewrite andbA; split; last by smt(size_rcons).
        split=> i ge0_i; rewrite size_rcons => ltsz1_i. 
...
seq 1 1 : (   #pre
           ...
           /\ i{1} = i0{2}).
+ by call(: true); skip.
case (0 <= i{1} < size O_MEFGCMA_WOTSTWES.qs{1}); last first.
+ conseq (: _ ==> true) => [/#| ]. 
...
while{1} (   #pre
          ...
          /\ (forall (i : int), 0 <= i < size pkWOTS0{1} =>
                ...
                = 
                cf ps1{1} (set_chthidx ad1{1} i) (BaseW.val em0{1}.[i])
...
pose k := find_collidx_r ps{1} (set_chthidx (nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`1 i)
           (BaseW.val (encode_msgWOTS (nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`2).[i])
           (BaseW.val (encode_msgWOTS m'{1}).[i])
           (val (nth witness (val (nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`4) i))
           (val (nth witness (val sig'{1}) i)).
have rng_i: 0 <= i < len by apply hchwcoll_findchrng.
move/allP: (qsch) => qsch'; move: (qsch' ((nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`1) _) => //.
+ by rewrite mapP; exists (nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}); rewrite mem_nth.
move => adch_i0.
have eq_cf : 
  cf ps{1} (set_chthidx (nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`1 i)
     (BaseW.val (encode_msgWOTS (nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`2).[i])
     (w - 1 - BaseW.val (encode_msgWOTS (nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`2).[i])
     (val (nth witness (val (nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`4) i)) =
  cf ps{1} (set_chthidx (nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`1 i)
     (BaseW.val (encode_msgWOTS m'{1}).[i])
...
+ move: (pkpval i _); first by smt().
  ...
  move: (pkvs (nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}) _); first by rewrite mem_nth => /#.
...
have neq0_em: BaseW.val (encode_msgWOTS (nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`2).[i] <> 0 by smt(BaseW.valP).
have ->:
  (nth witness O_SMDTTCR_Default.ts{1} (qsdgtcr_idx O_MEFGCMA_WOTSTWES.qs{1} R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{1} ps{1} i0{1} i (j + 1))).`2
  =
  nth witness (unzip2 O_SMDTTCR_Default.ts{1}) (qsdgtcr_idx O_MEFGCMA_WOTSTWES.qs{1} R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{1} ps{1} i0{1} i (j + 1)).
...
have ->:
  nth witness (unzip2 O_SMDTTCR_Default.ts{1}) 
              (qsdgtcr_idx O_MEFGCMA_WOTSTWES.qs{1} R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{1} ps{1} i0{1} i (j + 1))
  =
  val (extr_coll_l ps{1} (set_chthidx (nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`1 i)
              (BaseW.val (encode_msgWOTS (nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`2).[i])
              (BaseW.val (encode_msgWOTS m'{1}).[i])
              (val (nth witness (val (nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`4) i))
              (val (nth witness (val sig'{1}) i))).
+ rewrite rcqsdg nth_relcqsdgtcr_qsdgtcridx // 1:/# neq0_em /= /extr_coll_l -/cf -/j /=. 
  rewrite (Ring.IntID.addrC j 1) /cf ch_comp 1:ischtype_setchidx 3:valP //=; first 3 smt(BaseW.valP).
  congr; rewrite /sigvals allP /= in sigvs.
  move: (sigvs (nth (witness, witness) 
               (zip O_MEFGCMA_WOTSTWES.qs{1} R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{1}) i0{1}) _). 
  - by rewrite mem_nth; smt(size_zip).
  move/allP => /= h; move: (h i _); first by rewrite mem_range.
  by rewrite ?nth_zip // neq0_em /= => ->.
have ->: 
  (nth witness O_SMDTTCR_Default.ts{1}
     (qsdgtcr_idx O_MEFGCMA_WOTSTWES.qs{1} R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{1} ps{1} i0{1} i (j + 1))).`1
  =
  nth witness (unzip1 O_SMDTTCR_Default.ts{1}) (qsdgtcr_idx O_MEFGCMA_WOTSTWES.qs{1} R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{1} ps{1} i0{1} i (j + 1)).
+ rewrite(nth_map witness witness) //.
  rewrite (: size O_SMDTTCR_Default.ts{1} = size (unzip2 O_SMDTTCR_Default.ts{1})) 1:size_map //. 
  by rewrite rcqsdg qsdgtcridx_rng // neq0_em /#.
have {1}-> :
  nth witness (unzip1 O_SMDTTCR_Default.ts{1}) (qsdgtcr_idx O_MEFGCMA_WOTSTWES.qs{1} R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{1} ps{1} i0{1} i (j + 1))
  = 
  (set_htbidx (set_chthidx ((nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`1) i) ((BaseW.val (encode_msgWOTS (nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`2).[i]) + j)).
+ by rewrite rcqsad -eq_qsadtcridx_qsdgtcridx 3:nth_relcqsadtcr_qsadtcridx // neq0_em /= /#.
have {1}->: 
  nth witness (unzip1 O_SMDTTCR_Default.ts{1}) (qsdgtcr_idx O_MEFGCMA_WOTSTWES.qs{1} R_SMDTTCRC_Game34WOTSTWES.O_R_SMDTTCRC_Game34WOTSTWES.xll{1} ps{1} i0{1} i (j + 1))
  =
  (set_htbidx (set_chthidx ((nth witness O_MEFGCMA_WOTSTWES.qs{1} i0{1}).`1) i) ((BaseW.val (encode_msgWOTS m'{1}).[i]) + k)).
+ by rewrite rcqsad -eq_qsadtcridx_qsdgtcridx 3:nth_relcqsadtcr_qsadtcridx // neq0_em /= /#.
by apply collision_extraction.
qed.


(* - Fifth step: Reduce/Bound success probability in Game4_WOTSTWES - *)
...
swap{2} 13 -11.
wp => /=.
seq 9 12 : (   ={glob A, glob O_THFC_Default, O_MEFGCMA_WOTSTWES.qs, ps}
            /\ pp{2} = ps{2}
            /\ O_MEFGCMA_WOTSTWES.ps{1} = ps{1}
            /\ O_THFC_Default.pp{1} = ps{1}
            /\ O_THFC_Default.pp{2} = ps{2}
...
seq 1 1 : (   ={glob A, O_MEFGCMA_WOTSTWES.qs, ps}
           /\ pp{2} = ps{2}
           /\ all (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => is_chtype admpksig.`1) O_MEFGCMA_WOTSTWES.qs{1}
           /\ unzip1 O_SMDTPRE_Default.ts{2} = relcqsad_pre O_MEFGCMA_WOTSTWES.qs{1}
           /\ unzip2 O_SMDTPRE_Default.ts{2} = relcqsdg_pre O_MEFGCMA_WOTSTWES.qs{1}
           /\ (uniq_preffour (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) 
                 O_MEFGCMA_WOTSTWES.qs{1})
               => 
               disj_preffour (map (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => admpksig.`1) 
                 O_MEFGCMA_WOTSTWES.qs{1}) O_THFC_Default.tws{1} 
               => 
               disj_lists (unzip1 O_SMDTPRE_Default.ts{2}) O_THFC_Default.tws{2})).
+ call (:   ={O_MEFGCMA_WOTSTWES.qs, O_THFC_Default.pp}
         /\ O_MEFGCMA_WOTSTWES.ps{1} = O_SMDTPRE_Default.pp{2}
         /\ O_THFC_Default.pp{1} = O_SMDTPRE_Default.pp{2}
         /\ all (fun (admpksig : adrs * msgWOTS * pkWOTS * sigWOTS) => is_chtype admpksig.`1) O_MEFGCMA_WOTSTWES.qs{1}
         /\ unzip1 O_SMDTPRE_Default.ts{2} = relcqsad_pre O_MEFGCMA_WOTSTWES.qs{1}
         /\ unzip2 O_SMDTPRE_Default.ts{2} = relcqsdg_pre O_MEFGCMA_WOTSTWES.qs{1}
         /\ R_SMDTPREC_Game4WOTSTWES.O_R_SMDTPREC_Game4WOTSTWES.adl{2} = reltqsad_pre O_MEFGCMA_WOTSTWES.qs{1}
  ...
  - skip => /> _ _ _ tws qs ts tws' qsch rcqsad rcqsdg twspmem uqpfqs /hasPn djpfqs.
    ...
    * rewrite hasPn => ad; rewrite rcqsad /relcqsad_pre -flatten_mapP => -[q] [qin /=].
      ...
      case (BaseW.val (encode_msgWOTS q.`2).[i] <> 0) => //= neq0_em adval.
      move: (djpfqs (get_preffour ad)).
      rewrite -map_comp /(\o) /= => /(_ _). 
      + rewrite mapP; exists q; rewrite qin /= adval /get_preffour /set_htbidx /set_chthidx.
        by rewrite ?getadrsK // -madrs_dom /valid_adidxs /=; smt(allP madidxs_valid dist_adrstypes BaseW.valP).
  ...
  - proc; inline *.
    ...
    while{1} (   0 <= size pk{1} <= len
              ...
              /\ (forall (i : int), 0 <= i < size pk{1} =>
                    ...
                    nth witness pk{1} i
                    =
                    cf O_MEFGCMA_WOTSTWES.ps{1} (set_chthidx ad{1} i) 
    ...
    * move=> _ z.
    ...
    while (   ={ad, m, em, sig}
           ...
           /\ unzip1 O_SMDTPRE_Default.ts{2} 
              = 
              relcqsad_pre O_MEFGCMA_WOTSTWES.qs{1} ++ relcqsad_pre_outer ad{2} m{2} (size pk{2})
           /\ unzip2 O_SMDTPRE_Default.ts{2} 
              = 
              relcqsdg_pre O_MEFGCMA_WOTSTWES.qs{1} ++ relcqsdg_pre_outer m{2} sig{2} (size pk{2})
           /\ R_SMDTPREC_Game4WOTSTWES.O_R_SMDTPREC_Game4WOTSTWES.adl{2}
              =
              reltqsad_pre O_MEFGCMA_WOTSTWES.qs{1} ++ reltqsad_pre_outer ad{2} m{2} (size pk{2})
           ...
           /\ (forall (i : int), 0 <= i < size pk{2} =>
                ...
                nth witness pk{2} i
                =
                cf O_THFC_Default.pp{2} (set_chthidx ad{2} i) 
                  em_ele (w - 1 - em_ele) (val sig_ele))).
    * wp => /=.
      while{2} (   ={ad}
                /\ is_chtype ad{2}
                ...
                /\ R_SMDTPREC_Game4WOTSTWES.O_R_SMDTPREC_Game4WOTSTWES.adl{2}
                   =
                   reltqsad_pre O_MEFGCMA_WOTSTWES.qs{1} 
                   ++ reltqsad_pre_outer ad{2} m{2} (size pk{2})
                   ++ reltqsad_pre_inner (set_chthidx ad{2} (size pk{2})) em_ele{2} j{2}
                /\ size sig{1} = size pk{2}
                /\ (forall (ad : adrs), ad \in O_THFC_Default.tws{2} =>
                     ad \in O_THFC_Default.tws{1} \/
                     ad \in R_SMDTPREC_Game4WOTSTWES.O_R_SMDTPREC_Game4WOTSTWES.adl{2})
      ...
      + move=> &1 z.
        ...
        split; first by rewrite valP -/f eq_sym /cf chS 1:ischtype_setchidx 3:valP //; smt(BaseW.valP).
      ...
      move=> cel celin; split => [| _]; first by apply dmap_supp.
      rewrite valKd /= /cf ch0 1:ischtype_setchidx 3:valP //= {1}/reltqsad_pre_inner mkseq0 cats0 valKd /=.
      split => [| tws' j]; first by smt(BaseW.valP).
      split => [/# | /lezNgt gew1em_j twspmem ge0_j lew1em_j].
      split => [/# | neq0p_em].
      rewrite !andbA -andbA; split; last by smt(size_rcons).
      ...
      rewrite -!andbA; split.
      + by congr; rewrite eq_sz /cf ch1 1:ischtype_setchidx 3:valP //; smt(BaseW.valP).
    ...
    * rewrite rcqsad rcqsdg /relcqsad_pre_outer /relcqsdg_pre_outer /reltqsad_pre_outer.
      by rewrite 3!mkseq0 2!flatten_nil 3!cats0 /=; smt(ge2_len ChainingAddress.valP).
    ...
    * apply (eq_from_nth witness) => [/# | i rng_i].
    ...
    by rewrite allP => q; rewrite mem_rcons /= => -[-> |] //=; smt(allP ChainingAddress.valP).
  proc; inline *.
...
split => [| /#].
pose q := nth witness O_MEFGCMA_WOTSTWES.qs{2} i0{2}; rewrite eq_sym in neqq2_mp.
...
+ by apply hchwpre_findprerng.
rewrite /cf chS 1:ischtype_setchidx 3:valP //; first 5 smt(all_nthP BaseW.valP).
...
lemma MEFGCMA_WOTSTWES_NOPRF &m :
     Pr[M_EF_GCMA_WOTSTWES(A, O_MEFGCMA_WOTSTWES_NOPRF, O_THFC_Default).main() @ &m : res] 
  <=    (w - 2)%r
...
+ by byequiv MEFGCMA_WOTSTWES_NOPRF_Game2_WOTSTWES. 
have ->:
  Pr[Game2_WOTSTWES.main() @ &m : res]
  =
  `|Pr[Game2_WOTSTWES.main() @ &m : res] - 
...
lemma MEFGCMA_WOTSTWES &m :
    Pr[M_EF_GCMA_WOTSTWES(A, O_MEFGCMA_WOTSTWES, O_THFC_Default).main() @ &m : res] 
    <=
  ...
  `|Pr[SM_DT_UD_C(R_SMDTUDC_Game23WOTSTWES(A), O_SMDTUD_Default, O_THFC_Default).main(false) @ &m : res] - 
    ...
    Pr[SM_DT_PRE_C(R_SMDTPREC_Game4WOTSTWES(A), O_SMDTPRE_Default, O_THFC_Default).main() @ &m : res].
proof.
move: (MEFGCMA_WOTSTWES_NOPRF &m) => thm_nprf.
have ->:
  Pr[M_EF_GCMA_WOTSTWES(A, O_MEFGCMA_WOTSTWES, O_THFC_Default).main() @ &m : res] 
  = 
  Pr[Game0_WOTSTWES.main() @ &m : res].
+ by byequiv MEFGCMA_Game0_WOTSTWES.
have ->:
  Pr[Game0_WOTSTWES.main() @ &m : res]
  =
  `|Pr[Game0_WOTSTWES.main() @ &m : res] - 
...
apply (ler_trans (  `|Pr[Game0_WOTSTWES.main() @ &m : res] -
                      Pr[M_EF_GCMA_WOTSTWES(A, O_MEFGCMA_WOTSTWES_NOPRF, O_THFC_Default).main() @ &m : res]|
                  +   Pr[M_EF_GCMA_WOTSTWES(A, O_MEFGCMA_WOTSTWES_NOPRF, O_THFC_Default).main() @ &m : res])).
+ rewrite -{4}(ger0_norm Pr[M_EF_GCMA_WOTSTWES(A, O_MEFGCMA_WOTSTWES_NOPRF, O_THFC_Default).main() @ &m : res]). 
  - by rewrite Pr[mu_ge0]. 
  by apply ler_norm_add.
by rewrite -2!addrA ler_add 1:Step_Game0_MEFGCMA_WOTSTWES_NOPRF_PRF // addrA.
qed.

end section Proof_M_EF_GCMA_WOTSTWES.
